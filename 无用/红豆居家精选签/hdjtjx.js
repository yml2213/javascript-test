...
=============青龙变量实例=============
export hdqdbd='{"zhanId":635467,"templateId":4588,"extendInfo":{"isPersonal":{"enable":false},"youshu":{"enable":false}},"env":"production","wxTemplateId":5347,"storeId":2156307879,"appid":"wx884b3468051a7238","pid":"100000960879","refer":"","openid":"ogc7d4kweWX_YeeRG3173U6F9Jx8","source":1,"sdpSource":"ec","extendParameter":{"tsoTicket":"","wmkTicket":"","qwzsKey":""}}'
export wxtoken='jsc2skp.b60e59bd-fb0e-4f1c-a0df-f892439c69b4'
=============变量解释==========
hdqdbd: 可以从 关键词 activity/c/sign 包里找到所有变量
wxtoken: 可以从 关键词 activity/c/sign 包里找到所有变量
...
const $ = new Env('红豆居家精选签到');
const host = 'mapi.weimob.com';
const notify = $.isNode() ? require('./sendNotify') : '';
let wxtoken = process.env.wxtoken;
let body = process.env.hdqdbd;
...
async function yml() {
    ...
    function qd(timeout = 0) {
        return new Promise((resolve) => {
            let url = {
                url: `https://${host}/api3/misc/sign/activity/c/sign`,
                headers: {
            ...
            $.post(url, async (err, resp, data) => {
                try {
                    ...
                    if (result.errcode === 0) {
                        $.log(`\n【🎉🎉🎉 恭喜您鸭 🎉🎉🎉】执行签到: ${result.errmsg} ✅ 了呢 , ${result.data.forwardMsg}`)
                        await $.wait(3 * 1000)
                    } else {
                        $.log(`\n【🎉 恭喜个屁 🎉】执行签到:失败 ❌ 了呢,原因可能是 ${result.errmsg}!`)
                    }
                } catch (e) {
                    $.logErr(e, resp);
                } finally {
