...
const $ = new Env('东方头条大转盘');
const host = 'zhuanpan.dftoutiao.com';
const notify = $.isNode() ? require('./sendNotify') : '';
let dfttua = process.env.dfttua;
let body = process.env.dfttbxbd;
...
function dzp(timeout = 0) {
	return new Promise((resolve) => {
		let url = {
			url: `https://${host}/zhuanpan_v3/get_zhuanpan_new`,
			headers: {
				'User-Agent': dfttua
			},
			body: body
...
function dzpbx(timeout = 0) {
	return new Promise((resolve) => {
		let url = {
			url: `https://${host}/zhuanpan_v3/get_chest`,
			headers: {
				'User-Agent': dfttua
			},
			body: body
