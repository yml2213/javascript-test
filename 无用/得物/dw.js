/**
 * 得物 
 * cron 18 7 * * *  yml_javascript/dw.js
 * 
 * 得物 微信小程序 
 ...
 *  --- 重写 ---
 * https://qualcomm.growthideadata.com/qualcomm-app/api/activity url script-request-header https://raw.githubusercontent.com/yml2213/javascript/master/dw/dw.js
 * 
...
let dwCookies = ($.isNode() ? process.env.dwCookies : $.getdata('dwCookies')) || "";
let dwCookiesArr = [];
let msg = '';
...
// // UA判断部分
// if (!process.env.dw_UA) {
// 	console.log(`\n【${$.name}】：未填写 dw_UA 变量,将默认分配一个UA`);
...
!(async () => {
	if (typeof $request !== "undefined") {
		...
		if (!(await Envs()))  //多账号分割 判断变量是否为空  初步处理多账号
			...
			for (let index = 0; index < dwCookiesArr.length; index++) {


				data = dwCookiesArr[index].split('&')
...
async function Envs() {
	if (dwCookies) {
		if (dwCookies.indexOf("@") != -1) {
			dwCookies.split("@").forEach((item) => {
				dwCookiesArr.push(item);
			});
		} else {
			dwCookiesArr.push(dwCookies);
		}
	} else {
		$.log(`\n【${$.name}】：未填写变量 dwCookies`)
		return;
	}
...
async function GetRewrite() {
	if ($request.url.indexOf(`check`) > -1 && $request.headers) {
		...
		if (dwCookies != '') {
			if (dwCookies.indexOf(cookie) == -1) {
				dwCookies = dwCookies + '@' + cookie
				let List = dwCookies.split('@')

				$.setdata(dwCookies, 'dwCookies');
				$.msg(`【${$.name}】 获取第${List.length}个CK成功: ${cookie}`)
			} else {
				//$.msg($.name + ` 该账号CK已存在`)
			}
		} else {
			$.setdata(cookie, 'dwCookies');
