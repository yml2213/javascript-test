...
// 登录    post   https://member.kwwblcj.com/member/api/info/?userKeys=v1.0&pageName=loginFreePlugin&formName=searchForm&uid=9551915715434150&levelCode=1&redirect=https%3A%2F%2F89420.activity-20.m.duiba.com.cn%2Fprojectx%2Fp85657820%2Findex.html%3FappID%3D89420
async function login(name) {
	...
	try {
		let Options = {
			url: `https://member.kwwblcj.com/member/api/info/?userKeys=v1.0&pageName=loginFreePlugin&formName=searchForm&uid=${ck[0]}&levelCode=1&redirect=https%3A%2F%2F89420.activity-20.m.duiba.com.cn%2Fprojectx%2Fp85657820%2Findex.html%3FappID%3D89420`,
			headers: {
		...
		if (result.flag == 'T') {
			...
			let login_url = result.result;
			await get_cookie('获取cookie', login_url);
			await wait(2);
			return ck_status = 1;
		} else {
...
// 获取cookie   
async function get_cookie(name, login_url) {
	if (!name) {
		name = /function\s*(\w*)/i.exec(arguments.callee.toString())[1];
	}
	DoubleLog(`\n开始 ${name}`);
	try {
		...
		function callback(error, response, body) {
			if (!error && response.statusCode == 200) {
				...
				kww_cookie = response.request.headers.cookie
				DoubleLog(`ok`)
				// return kww_cookie
			}
		}
...
async function task_list(name) {
	...
	try {
		let Options = {
			url: `${hostname}/projectx/p85657820/customTask1/queryTasks.do`,
			headers: {
				'Host': host,
				'Cookie': kww_cookie,
			}
