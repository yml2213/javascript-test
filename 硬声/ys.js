/**
 * 脚本地址: http://yml-gitea.ml:2233/yml/JavaScript-yml/raw/branch/master/ys.js
 * 转载请留信息,谢谢
 *
 * 硬声  app 
 *
 * cron 15 7,12 * * *  yml2213_javascript_master/ys.js
 *
 * 6-9		自己修改版本,完善所有任务!
 *
 * 感谢所有测试人员
 * ========= 青龙--配置文件 =========
 * 变量格式: export ys_data='手机号&密码 @ 手机号&密码'  多个账号用 换行 或 @分割
 *
...
let key = 'q09cRVOPCnfJzt7p';
let iv = 'cnry8k3o4WdCGU1T';
let salt = 'lw0270iBJzxXdJLRtePEENsauRzkHSqm'
// let ts = ts10();
///////////////////////////////////////////////////////////////////
let VersionCheck = "0.0.1"
let Change = '自己修改版本,完善所有任务!'
let thank = `\n感谢 xx 的投稿`
///////////////////////////////////////////////////////////////////

async function tips(ckArr) {
	let Version_latest = await Version_Check('ys');
...
!(async () => {
	let ckArr = await getCks(ckStr, "ys_data");
	await tips(ckArr);
...
async function login() {
	let ts = ts10();
	let pwd = encodeURIComponent(getAesString(`${ck[1]}`, key, iv));
	let params_login = `Authorization=&account=${ck[0]}&device_id=ai25ccb75e67035fd95ff293d3f9efda87&password=${pwd}&platform=android&timestamp=${ts}${iv}${key}`;
	...
	let Options = {
		...
		body: `password=${pwd}&device_id=ai25ccb75e67035fd95ff293d3f9efda87&account=${ck[0]}`
	};
...
/**
 * 任务列表    httpGet  10,11 -- 未完成, 12 -- 待领取， 13 -- 已完成
...
async function task_list() {
	...
	let sign_task_list = sha1(`${salt}${sha1(params_task_list)}${AZ}`)
	let Options = {
...
async function recommendList() {
	...
	let result = await httpGet(Options, `推荐列表`);
...
*/
async function receiveCoin(task) {
	let ts = ts10();
	let data_ = `Authorization=${AZ}&platform=h5&timestamp=${ts}`;
	let sign = sha1(`${salt}${sha1(data_)}${AZ}`)
	let Options = {
	...
	let result = await httpPost(Options, `领取硬币`);

	if (result.code == 0) {
		DoubleLog(`领取任务 ${task.title} 奖励 ,获得 ${result.data.coins} 硬币!`);
		await wait(3);
	} else {
		DoubleLog(`领取任务 ${task.title} : 失败 ❌ 了呢 , ${result.message}`);
		console.log(result);
	}
}
...
/**
* 刷新看视频进度  httpPost 
* https://ysapi.elecfans.com/api/activity/task/viewVideo/add
27d420c3e5540b977747225565e5a436df9c34c7
*/
async function viewVideoAdd(step) {
	// let key = 'q09cRVOPCnfJzt7p';
	// let iv = 'cnry8k3o4WdCGU1T';
	// let salt = 'lw0270iBJzxXdJLRtePEENsauRzkHSqm'
...
/**
* 做任务  httpPost 
* https://yingsheng.elecfans.com/webapi/wapi/activity/task/receive
*/
async function taskReceive(task) {
	let ts = ts10();
	let data_ = `Authorization=${AZ}&platform=h5&timestamp=${ts}`;
	let sign = sha1(`${salt}${sha1(data_)}${AZ}`)
	let Options = {
		url: `https://yingsheng.elecfans.com/webapi/wapi/activity/task/receive`,
		headers: {
			"Host": "ysapi.elecfans.com",
			"authorization": AZ,
			"timestamp": ts,
	...
	if (result.code == 0) {
		DoubleLog(`做任务: ${task.title} 成功!`);
		await wait(3);
	} else {
		DoubleLog(`做任务: ${task.title}: 失败 ❌ 了呢 , ${result.message}`);
		console.log(result);
	}
}
...
async function like() {
	...
	let Options = {
		...
		body: JSON.stringify({ 'video_id': audio_id }),
	};
	let result = await httpPost(Options, `点赞`);

	if (result.code == 0) {
		DoubleLog(`点赞: 视频id ${video_id} ,点赞 ${result.data.msg}!`);
		await wait(3);
	} else {
		DoubleLog(`点赞: 视频id ${video_id}: 失败 ❌ 了呢 , ${result.message}`);
		console.log(result);
	}
}
...
async function follow(user_id, type) {
	...
	let result = await httpPost(Options, `关注`);

	let str = type == 1 ? '关注' : '取消关注';
	if (result.code == 0) {
		DoubleLog(`${str}用户[${user_id}]成功`);
		await wait(3);
	} else {
		DoubleLog(`${str}用户[${user_id}]失败: ${result.message}`);
		console.log(result);
	}
}
...
/**
 * 获取远程版本
 * http://yml-gitea.ml:2233/yml/JavaScript-yml/raw/branch/master/ys.js
 */
