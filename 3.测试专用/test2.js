/*
硬声 
23/2/3      修复旧列表只有一个, 导致无法领取关注任务的错误 -- yml
...
class UserInfo {
    ...
    // 旧 https://ysapi.elecfans.com/api/live/recommendList
    // 新 https://ysapi.elecfans.com/api/recommend/video/index?page=1
    // 修复旧列表只有一个, 导致无法领取关注任务的错误
    async recommendList() {  // 推荐列表  
        try {
            let param = { page: 1 }
            let url = `https://ysapi.elecfans.com/api/recommend/video/index?${$.json2str(param, '&')}`
            let body = ``
            param = this.calculateSign_ysapi(param)
            let urlObject = populateUrlObject(url, param, body)
            await httpRequest('get', urlObject)
