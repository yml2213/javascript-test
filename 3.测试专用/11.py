...
class Oshwhub:
    ...
    url_exchange = "https://oshwhub.com/api/gift/exchangeGift"
    ...
    def auto_sign(self):
        ...
        _oshwhub_session = str(_oshw_cookies).replace(
            "'", "").split(",")[1][17:]
        _oshwhubReferer = str(_oshw_cookies).replace(
            "'", "").split(",")[4][17:]
        ...
        login_res = requests.get(
            self.url_oshw2passport, headers=oshw_headers, allow_redirects=False
        )
        ...
        passport_res = requests.get(
            login_res.headers["Location"],
        ...
        passport_res = requests.get(
            passport_res.headers["Location"], headers=passport_headers2
        )
        SESSION = passport_res.headers["Set-Cookie"].split(
            ";")[-4].split("=")[-1]
        ...
        LT = re.findall(
            r'<input type="hidden" name="lt" value="(.*?)" />', passport_res.text
        )
        ...
        print("开始验证ticket...")
        try:
            oshw_res = requests.get(
                passport_res.headers["Location"],
                headers=oshw_headers,
        ...
        print("更新SESSION...")
        oshw_res = requests.get(
            oshw_res.headers["Location"],
            headers=oshw_headers,
            cookies=oshw_cookies,
            allow_redirects=False,
        )
        oshw_cookies["oshwhub_session"] = cookies2dict(oshw_res.headers["Set-Cookie"])[
            "oshwhub_session"
        ]
        ...
        oshw_sign = requests.post(
            self.url_signIn, headers=oshw_headers, cookies=oshw_cookies
        )
        oshw_cookies["oshwhub_session"] = cookies2dict(oshw_res.headers["Set-Cookie"])[
            "oshwhub_session"
        ]
        ...
        if not json.loads(oshw_sign.content)["code"]:
            ...
            self.sign_Statistics = "签到结果: " + \
                json.loads(oshw_sign.content)["message"]
        # 获取三天签到奖励
        oshw_res = requests.get(
            self.url_threeDay, headers=oshw_headers, cookies=oshw_cookies
        )
        oshw_cookies["oshwhub_session"] = cookies2dict(oshw_res.headers["Set-Cookie"])[
            "oshwhub_session"
        ]
        ...
        if not json.loads(oshw_res.content)["code"]:
            ...
            self.three_reward_Statistics = (
                "三天奖励领取结果: " + json.loads(oshw_res.content)["message"]
            )

        # 获取七日奖品信息
        oshw_res = requests.get(
            self.url_giftInfo, headers=oshw_headers, cookies=oshw_cookies
        )
        oshw_cookies["oshwhub_session"] = cookies2dict(oshw_res.headers["Set-Cookie"])[
            "oshwhub_session"
        ]
        ...
        coupon_uuid = json.loads(oshw_res.content)[
            "result"]["sevenDay"]["coupon_uuid"]
        coupon_name = json.loads(oshw_res.content)[
            "result"]["sevenDay"]["name"]

        coupon_data = {"gift_uuid": uuid, "coupon_uuid": coupon_uuid}
        # 领取七日奖励
        oshw_res = requests.post(
            self.url_sevenDay,
            data=coupon_data,
            headers=oshw_headers,
            cookies=oshw_cookies,
        ...
        if not json.loads(oshw_res.content)["code"]:
            self.seven_reward_Statistics = (
                "七天奖励领取成功, 奖品为: " +
                json.loads(oshw_res.content)["result"]["info"]
            )
        else:
            self.seven_reward_Statistics = (
                "七天奖励领取结果: " + json.loads(oshw_res.content)["message"]
            )

        # 兑换 万用表
        print('======================== 运行到我了 ========================')
        oshw_exchange = requests.post(
            self.url_exchange, headers=oshw_headers, cookies=oshw_cookies
        )
        oshw_cookies["oshwhub_session"] = cookies2dict(oshw_res.headers["Set-Cookie"])[
            "oshwhub_session"
        ]
        # print(oshw_cookies)
        if json.loads(oshw_exchange.content)["code"] != '100003':
            self.exchange_Statistics = "签到结果: 签到成功"
        else:
            self.exchange_Statistics = "签到结果: " + \
                json.loads(oshw_exchange.content)["message"]
...
if __name__ == "__main__":
    try:
        ...
        for key in users:
            ...
            print(my_user.exchange_Statistics)
