 ...
 *
 * ========= 青龙--配置文件 =========
 * 变量格式: export jtqcck='手机号 & 密码' 多账号@ 分割
 * 
 */
...
//---------------------------------------------------------------------------------------------------------
let ckStr = ($.isNode() ? process.env.jtqcck : $.getdata('jtqcck')) || ''
let msg, ck
let ck_status = true
...
!(async () => {
    let ckArr = await checkEnv(ckStr, "jtqcck")
    await tips(ckArr)
...
async function getkey() {
    let Option = {
        ...
        body: JSON.stringify({ "algorithm": "aes", "username": `${ck[0]}`, "terminal": 3 })
    }
    let result = await httpPost(Option, `申请key`)

    if (result.status == 200) {
        DoubleLog(`申请key:${result.message}`)
        key = result.data.random
        iv = result.data.iv
        await wait(2)
        // console.log(key, iv)
    } else {
        DoubleLog(`申请key: 失败 ❌ 了呢,原因未知!`)
        console.log(result)
    }
}
...
function aes_cbc(data, key, iv) {
    ...
    let encrypted = CryptoJS.AES.encrypt(password, key, { iv: iv, mode: CryptoJS.mode.CBC, padding: CryptoJS.pad.Pkcs7 })
    return encrypted + ""
}
...
async function login() {
    let pw_data = `${ck[1]}|${key}`
    // console.log(pw_data)
    let pwd = aes_cbc(pw_data, key, iv)
    let Option = {
        ...
        body: JSON.stringify({ "username": `${ck[0]}`, "password": `${pwd}`, "client_id": "bigapp", "source": 2, "deviceModel": "Redmi 22041211AC", "version": "3.2.2", "terminal": 3 })
    }
    // console.log(Option)
