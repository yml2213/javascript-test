/*
# 好悍邦
export  hhbbody='tgid@phone@pwd'
*/

NAME = `好悍邦`
VALY = ['hhbbody']
$ = TomEnv()
CK = '11@13754650804@hhb123456..'

DATA = ''

class Bar {
    constructor(str) {
        this.tgid = str.split('@')[0]
        this.userCode = str.split('@')[1]
        this.password = $.MD5Encrypt(str.split('@')[2])
        this.login = true
        this.signs = true
    }

    async userinfo() {
        await $.task(`post`, `https://jacsupperapp.jac.com.cn/api/jac-admin/admin/userBaseInformation/userLogin`, {}, `{"userType":"1","userCode":"${this.userCode}","password":"${this.password}"}`)
        if (DATA.code == 0) {
            this.login = true
            this.mobile = DATA.data.userCode
            this.no = DATA.data.no
            this.token = DATA.data.token
            console.log(`账号[${this.mobile}]  更新token：${this.no}`)
            this.headers = {
            ...
            let msg = encodeURIComponent(`${NAME} ${this.userCode} Cookie失效,请重新上传Cookie`)
        }
    }

    async sign() {
        await $.task(`post`, `https://jacsupperapp.jac.com.cn/api/pluto-membership/pluto-membership/integral-gather/addintegral-signIn`, this.headers, `{"ruleStr":"SIGN_IN","serviceTypeStr":"SERVICE_FIXED","no":"${this.no}"}`)
        if (DATA.code == 0) {
            console.log(`账号[${this.mobile}]  连续签到${DATA.data.num}天,获得积分：${DATA.data.integral}`)
            let msg = encodeURIComponent(`${NAME} 账号[${this.mobile}]  连续签到${DATA.data.num}天,获得积分：${DATA.data.integral}`)
        }
    }

    async check() {
        await $.task(`post`, `https://jacsupperapp.jac.com.cn/api/pluto-membership/plutomembership/integralCount/searchIntegralCountList`, this.headers, `{"no":"${this.no}"}`)
        if (DATA.code == 0) {

            console.log(`账号[${this.mobile}]  积分：${DATA.data[0].count}`)

        }
    }

    async Integral() {
        await $.task(`post`, `https://jacsupperapp.jac.com.cn/api/pluto-membership/pluto-membership/integral-gather/is-sign`, this.headers, `{"no":"${this.no}"}`)
        if (DATA.code == 0) {
            this.signs = false
            console.log(`账号[${this.mobile}] 今日未签到`)
        } else {
            this.signs = true
            console.log(`账号[${this.mobile}] ${DATA.msg}`)
        }
    }
...
!(async () => {
    ...
    console.log(`\n================ 登录账号 ================`)
    for (let user_a of CookieList) {
        await user_a.userinfo()
    }

    let validUserList = CookieList.filter(x => x.login == true)
    ...
    let signUserList = validUserList.filter(x => x.signs == false)
    if (signUserList.length >= 1) {
        console.log(`\n================== 签到 ==================`)
        for (let user_sign of signUserList) {
            await user_sign.sign()
        }
    }
})()
    .catch(e => {
    ...
    return new (class {
      ...
      */
        ...
        ExamineCookie() {
            ...
            if (ckStr) {
                for (let userCookies of ckStr.split('&').filter(x => !!x)) {
                    CookieList.push(new Bar(decodeURIComponent(userCookies)))
                }
                userCount = CookieList.length
            } else {
                console.log(`\n【${$.name}】：未填写变量: ${VALY}`)
            }
            ...
            return new Promise(async resolve => {
                ...
                request[method.toLowerCase()](httpget, (err, response, data) => {
                    try {
                        ...
                        console.log(`=======`)
                        if (err) {
                            console.log(taskurl + 'API请求失败，请检查网络重试' + err)
                        } else {
                            if (data) {
                                if ($.safeGet(data)) {
        ...
        MD5Encrypt(a) {
            ...
            return O.toLowerCase() //.toUpperCase() //.toUpperCase() 转大写
        }
        //SHA1
        SHA1Encrypt(msg) {
            function add(x, y) {
