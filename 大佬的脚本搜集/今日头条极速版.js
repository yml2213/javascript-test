...
let userAgent =
	...
	"NewsLite 8.5.8 rv:8.5.8.20 (iPhone; iOS 14.7.1; zh_CN) Cronet";
let userAgentArr = [];
let userHeader =
...
let userStatus = [];
let maxReadPerRun =
	($.isNode() ? process.env.jrttjsbReadNum : $.getdata("jrttjsbReadNum")) || 10;
let readList = [];

let validList = [];
let adIdList = [
	26, 181, 186, 187, 188, 189, 190, 195, 210, 214, 216, 225, 308, 324, 327, 329,
];
...
async function GetRewrite() {
	if ($request.url.indexOf("luckycat/lite/v1/task/page_data") > -1) {
		...
		if (userHeader) {
			if (userHeader.indexOf(userCK) == -1) {  // 找不到返回 -1
...
async function checkEnv() {
	if (userHeader) { // 空是 false  有数据是 true
		userHeaderArr = userHeader.split("@");  // 一般用数组储存变量，一般使用 @ 分割多账号 ; 使用 & 分割每个账号的数据
	} else {
...
async function initAccountInfo() {
	for (userIdx = 0; userIdx < userHeaderArr.length; userIdx++) {
		userStatus.push(true);  // 往数组中添加新元素，默认添加到最后面   userStatus = [true]
	}
}
...
async function ReadArtsReward() {
	...
	if (result.err_no == 0) {
		console.log(
			`用户${userIdx + 1}阅读文章获得${result.data.score_amount
...
async function DailyArtsReward() {
	...
	if (result.err_no == 0) {
		console.log(
			`用户${userIdx + 1}领取每日阅读奖励获得${result.data.score_amount}金币`
		);
	} else {
...
async function DailyPushReward() {
	...
	if (result.err_no == 0) {
		console.log(
			`用户${userIdx + 1}领取每日推送奖励获得${result.data.score_amount}金币`
		);
	} else {
...
async function SignIn() {
	...
	if (result.err_no == 0) {
		console.log(
			`用户${userIdx + 1}签到成功，获得${result.data.score_amount
...
async function ExcitationAd(task_id) {
	...
	if (result.err_no == 0) {
		console.log(
			`用户${userIdx + 1}看视频任务[${task_id}]获得${result.data.score_amount
			}金币`
		);
		...
		if (result.data.sleeping == true) {
			if (sleepHour >= 12) {
				...
				if (rnd > 0.95) {
					...
					console.log(
						`用户${userIdx + 1
						}随机醒来时间，本次不进行醒来，已经睡了${sleepHour}小时，可以获得${result.data.sleep_unexchanged_score
						}金币`
					);
				}
			} else {
				console.log(
					`用户${userIdx + 1}睡眠中，已经睡了${sleepHour}小时，可以获得${result.data.sleep_unexchanged_score
...
async function SleepStop() {
	...
	if (result.err_no == 0) {
		...
		console.log(
			`用户${userIdx + 1}结束睡眠，本次睡了${sleepHour}小时，可以领取${result.data.history_amount
			}金币`
		);
		await SleepDone(result.data.history_amount);
	} else {
...
async function RewardFarmThreeGift(gift_id) {
	...
	if (result.status_code == 0) {
		console.log(
			`用户${userIdx + 1}领取农场三餐礼包获得${result.data.reward_num}水滴`
		);
	} else {
...
async function RewardFarmTask(id) {
	...
	if (result.status_code == 0) {
		...
		console.log(
			`用户${userIdx + 1}领取农场任务奖励[task_id=${result.data.task_id}]获得${result.data.reward_num
			}${typeStr}，剩余${typeStr}数量${result.data.current_num}`
		);
	} else {
...
async function FarmSign() {
	...
	if (result.status_code == 0) {
		...
		console.log(
			`用户${userIdx + 1}签到获得${result.data.reward_num
...
async function FarmSignDouble() {
	...
	if (result.status_code == 0) {
		let str = result.data.reward_type == 1 ? "水滴" : "化肥";
		console.log(
			`用户${userIdx + 1}签到翻倍获得${result.data.reward_num
			}{str}，剩余${str}数量${result.data.cur_reward_num}`
		);
	} else {
...
async function TreeSign() {
	...
	if (result.status_code == 0) {
		console.log(
			`用户${userIdx + 1}种树签到获得${result.data.reward_item.num}${result.data.reward_item.name
			}`
		);
	} else {
...
async function TreeChallengeReward() {
	...
	if (result.status_code == 0) {
		console.log(
			`用户${userIdx + 1}领取浇水挑战奖励获得${result.data.reward_item.num}水滴`
		);
	} else {
...
async function TreeNutrientSign() {
	...
	if (result.status_code == 0) {
		if (result.data.is_rewarded == true) {
			console.log(
				`用户${userIdx + 1}种树化肥签到获得${result.data.reward_item.num
...
async function RewardTreeThreeGift(task_id) {
	...
	if (result.status_code == 0) {
		console.log(
			`用户${userIdx + 1}领取种树三餐礼包获得${result.data.reward_item.num}${result.data.reward_item.name
			}`
		);
	} else {
...
async function TreeWaterReward(task_id) {
	...
	if (result.status_code == 0) {
		console.log(
			`用户${userIdx + 1}领取水滴任务[id=${task_id}]获得${result.data.reward_item.num
			}${result.data.reward_item.name}`
		);
	} else {
		console.log(
			`用户${userIdx + 1}领取水滴任务[id=${task_id}]失败：${result.message}`
		);
	}
}
...
async function TreeWater() {
	...
	if (result.status_code == 0) {
		console.log(
			`用户${userIdx + 1}种树浇水成功，剩余水滴：${result.data.kettle.water_num
			}`
		);
...
async function TreeWaterTenTimes() {
	...
	if (result.status_code == 0) {
		console.log(
			`用户${userIdx + 1}种树浇水10次成功，剩余水滴：${result.data.kettle.water_num
			}`
		);
		if (result.data.kettle.water_num >= 100) {
			await $.wait(1500); //min time 1000
...
async function RewardTreeWaterBottle() {
	...
	if (result.status_code == 0) {
		console.log(
			`用户${userIdx + 1}领取种树水瓶奖励获得${result.data.reward_item.num}水滴`
		);
	} else {
