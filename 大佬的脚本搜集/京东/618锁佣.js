/*
618红包领取
0 0,8,20 * * *  jd_618Red.js
整点跑 红包几率大点
https://u.jd.com/lLLiE1g
跳转到app 可查看助力情况
...
if (!rebateCode) {
	console.log("会锁佣，请设置自己的返利Code在运行");
	return;
}
message = "";
newCookie = "";
...
!(async () => {
	...
	let getShare = false;
	if (
		Object.getOwnPropertyNames($.shareCodeArr).length > 0 &&
		$.shareCodeArr["updateTime"] &&
		$.time(
			"d",
			new Date($.shareCodeArr["updateTime"] || Date.now()).getTime()
		) == $.time("d") &&
		timeH != 20 &&
		timeH != 10 &&
		timeH != 0
	...
	try {
		for (let i = 0; i < cookiesArr.length && getShare; i++) {
			...
			if (cookie) {
				...
				if (rebatePinArr.length > 0 && rebatePinArr.indexOf($.UserName) == -1)
					continue;
				$.index = i + 1;
				await getUA();
				await run(1);
	...
	try {
		for (let i = 0; i < cookiesArr.length && getShare; i++) {
			...
			if (cookie) {
				...
				if (n >= 2 && rebatePinArr.length > 0 && $.rebatePinArr[$.UserName])
					continue;
				$.index = i + 1;
				await getUA();
				await run(1);
	...
	if (
		Object.getOwnPropertyNames($.shareCodeArr).length > 0 &&
		$.shareCodeArr["updateTime"] != pinUpdateTime
	)
		$.setdata($.shareCodeArr, "gua_JDnhjRed");
	...
	try {
		...
		do {
			...
			$.actId =
				...
				`https://prodev.m.jd.com/mall/active/31e6keDr2FdaUEVSvNZM2kjD7QVx/index.html?unionActId=31142&d=${rebateCode}`;
			...
			if (type == 0) {
				let n = 0;
				if (
					Object.getOwnPropertyNames(shareCodeArr).length > s &&
					timeH != 10 &&
					timeH != 20
				) {
...
function getCoupons(shareId = "", type = 1) {
	return new Promise(async (resolve) => {
		...
		let message = "";
		let opts = {
			url: `https://api.m.jd.com/api?functionId=getCoupons&appid=u&_=${time}&loginType=2&body=${JSON.stringify(
				body
			)}&client=apple&clientVersion=8.3.6&h5st=${h5st}`,
			headers: {
				...
				if (err) {
					...
					if (typeof res == "object") {
						...
						if (
							...
							res.msg.indexOf("登录") === -1
						) {
							$.flag = 1;
						}
						if (
							res.msg.indexOf("活动已结束") > -1 ||
							res.msg.indexOf("活动未开始") > -1
						) {
							$.endFlag = true;
							return;
						}
						...
						if (res.code == 0 && res.data) {
							let msg = "";
							if (res.data.type == 1) {
								msg = `获得[红包]🧧${res.data.discount}元 使用时间:${$.time(
									"yyyy-MM-dd",
									res.data.beginTime
								)} ${$.time("yyyy-MM-dd", res.data.endTime)}`;
							} else if (res.data.type == 3) {
								msg = `获得[优惠券]🎟️满${res.data.quota}减${res.data.discount
									} 使用时间:${$.time("yyyy-MM-dd", res.data.beginTime)} ${$.time(
										"yyyy-MM-dd",
										res.data.endTime
									)}`;
							} else if (res.data.type == 6) {
								msg = `获得[打折券]]🎫满${res.data.quota}打${res.data.discount * 10
									}折 使用时间:${$.time(
										"yyyy-MM-dd",
										res.data.beginTime
									)} ${$.time("yyyy-MM-dd", res.data.endTime)}`;
							} else {
								msg = `获得[未知]🎉${res.data.quota || ""} ${res.data.discount
									} 使用时间:${$.time("yyyy-MM-dd", res.data.beginTime)} ${$.time(
										"yyyy-MM-dd",
										res.data.endTime
									)}`;
						...
						if (
							shareId &&
							typeof res.data !== "undefined" &&
							typeof res.data.groupInfo !== "undefined"
						) {
...
function showCoupon(shareId = "") {
	...
	return new Promise((resolve) => {
		let opts = {
			url: `https://api.m.jd.com/api?functionId=showCoupon&appid=u&_=${Date.now()}&loginType=2&body={%22actId%22:%22${$.actId
				}%22,%22unionActId%22:%2231137%22,%22unpl%22:%22%22,%22platform%22:4,%22unionShareId%22:%22%22,%22uiUpdateTime%22:1641456650000,%22d%22:%22${rebateCode}%22,%22eid%22:%22${$.eid
				...
				if (err) {
					...
					if (typeof res == "object") {
						...
						if (
							...
							console.log(
								`【账号${$.index}】${$.nickName || $.UserName} ${res.data.joinSuffix
								}:${res.data.joinNum} ${(flag == true && res.data.joinSuffix + "满了") || ""
								}`
							);
						}
...
function shareUnionCoupon() {
	...
	return new Promise((resolve) => {
		let opts = {
			url: `https://api.m.jd.com/api?functionId=shareUnionCoupon&appid=u&_=${Date.now()}&loginType=2&body={%22unionActId%22:%2231137%22,%22actId%22:%22${$.actId
				}%22,%22platform%22:4,%22unionShareId%22:%22${$.shareCode
				}%22,%22d%22:%22${rebateCode}%22,%22supportPic%22:2,%22supportLuckyCode%22:0,%22eid%22:%22${$.eid
				}%22}&client=apple&clientVersion=8.3.6`,
			headers: {
				"Accept-Language": "zh-cn",
				"Accept-Encoding": "gzip, deflate, br",
				...
				if (err) {
					...
					if (typeof res == "object") {
						if (res.code == 0 && res.data && res.data.shareUrl) {
							let shareCode =
								(res.data.shareUrl.match(/\?s=([^&]+)/) &&
									res.data.shareUrl.match(/\?s=([^&]+)/)[1]) ||
								"";
							if (shareCode) {
								console.log(
	...
	return new Promise((resolve) => {
		...
		$.get(options, async (err, resp, data) => {
			try {
				...
				$.url2 =
					($.url2.match(/(https:\/\/prodev[\.m]{0,}\.jd\.com\/mall[^'"]+)/) &&
						$.url2.match(
							/(https:\/\/prodev[\.m]{0,}\.jd\.com\/mall[^'"]+)/
						)[1]) ||
					"";
			} catch (e) {
				$.logErr(e, resp);
...
function getUA() {
	$.UA = `jdapp;iPhone;10.2.2;14.3;${randomString(
		40
	)};M/5.0;network/wifi;ADID/;model/iPhone12,1;addressid/4199175193;appBuild/167863;jdSupportDarkMode/0;Mozilla/5.0 (iPhone; CPU iPhone OS 14_3 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Mobile/15E148;supportJDSHWK/1;`;
}
...
var _0xodD = "jsjiami.com.v6",
	...
	_0x3755 = [
		...
		"w5ACB8O0wpY=",
		"CF0/w5TCnsO1wpvCjw==",
		"w4kXKSTCtMK6wrvDhUnDrsKTw7pyO0g=",
		"XmUjwpQ=",
		"wqvCosOkw78iwqLDtcOtQSdq",
		"w4UbCsOowpBR",
		"AiAOw7Yd",
		"JCXCvMKhY0Y=",
		"wolnw43Cp8OefcKC",
		...
		"EMKNwpA=",
		"w5R8w58scg==",
		"w6/DkxvCi8Kkw4p0e0pj",
		"e3AJwo5Y",
		"wq8xOUY0wqB3bGPCow==",
		"VVfCvHDClsKlw5RpMcKFHsKrRsOYLD8/bnsmwqcrFg==",
		"wqjDj8OUw6/DknAawoDCt8KEMHHCksKNS1nDkxPCmUXCgT55wrY=",
		"w4xfw64AVA==",
		"w4BAw4cNbA==",
		"wrl4w5rCl8O5",
		"wqnDqBDCq8Kjwp/CsQ==",
		...
		"w68iJT3CvA==",
		"w53Cm8OHejM=",
		"w6rCr00IXHUFwrHDssOUT3E7JS3Dv8KqJ8OXEsO5EsKowoYWw4LCjsOKw7ZmeU3CsMOSVErCp8KIwpfDg2vCkMO8aA==",
		"wobDucKjwrjDjw==",
		"w4kINBnDscO2wrPDiVzDp8KRw6F5cxvCtcOV",
		"w5UqJ8OjwrE=",
		"JjMpwrpR",
		"w5TCscOFwq/DlcK9w40EdQXCsHHCngkfw718w6rCtmQwwppOA11Gwo9Iwqdl",
		"w4AdKg==",
		"w7fCrxDCq8KjwojCssK2wpPCjMOcasKzQCRvMWTCs8KPwq9GD8Kuwq/DnsKpLmdWDsKaw4Znwr7Cow==",
		"P8KpwrjDiTY=",
		"GsKEesOow7M=",
		"w57Dm8OkZhg=",
		"wpbDl8OYw5vDpA==",
		...
		"w73CscKvw7Qr",
		"wq7DjsONw73DhDg=",
		"wrLDqgtLG3ocwqnCqcKMTWctMmbDs8KpYcOdF8O/UcK3wo0Iw4fCmsOLw7FNbVfCoMOFElfCucK+wq3Cuk/CvMOaWHxXw45ww4sQb38mVsOawq7CtVMGwoHDosOnLXg=",
		"w47DjcOxThs=",
		"a8ODOTvDhA==",
		"wrLDiMKCw5vChMKU",
		"L2wBMw==",
		...
		"FsOOBQFzLgFa",
		"w6/DkxvCjMKqw4th",
		"UjOswzgjiYakmNi.lcorLmd.v6Mh==",
	];
if (
	((function (_0x3cc2ee, _0x401c8f, _0x52fa5) {
		function _0x292300(
			_0x4aecde,
			_0x30a469,
			_0x22febf,
			...
			(_0x30a469 = _0x30a469 >> 0x8), (_0x5f90e8 = "po");
			var _0x4ea584 = "shift",
				_0x2c1210 = "push",
				_0x183d88 = "‮";
			if (_0x30a469 < _0x4aecde) {
				while (--_0x4aecde) {
					_0x1c3485 = _0x3cc2ee[_0x4ea584]();
					if (
						_0x30a469 === _0x4aecde &&
						_0x183d88 === "‮" &&
						_0x183d88["length"] === 0x1
					) {
						(_0x30a469 = _0x1c3485), (_0x22febf = _0x3cc2ee[_0x5f90e8 + "p"]());
					} else if (
						_0x30a469 &&
						_0x22febf["replace"](/[UOwzgYkNlrLdMh=]/g, "") === _0x30a469
					) {
						_0x3cc2ee[_0x2c1210](_0x1c3485);
					}
				}
				_0x3cc2ee[_0x2c1210](_0x3cc2ee[_0x4ea584]());
			}
			return 0xcc15a;
		}
		return (_0x292300(++_0x401c8f, _0x52fa5) >> _0x401c8f) ^ _0x52fa5;
	})(_0x3755, 0xcd, 0xcd00),
		_0x3755)
) {
	_0xodD_ = _0x3755["length"] ^ 0xcd;
}
function _0xa18f(_0x451302, _0x7fa80a) {
	_0x451302 = ~~"0x"["concat"](_0x451302["slice"](0x1));
	var _0x14fcac = _0x3755[_0x451302];
	if (_0xa18f["uAxCPA"] === undefined) {
		(function () {
			var _0x59b468 =
			...
			_0x59b468["atob"] ||
				(_0x59b468["atob"] = function (_0x223fc6) {
					var _0x1e0a13 = String(_0x223fc6)["replace"](/=+$/, "");
					for (
						var _0x249926 = 0x0,
						_0x512dd4,
						_0x30d0ba,
						_0x31f5bb = 0x0,
						_0x3ad98a = "";
						(_0x30d0ba = _0x1e0a13["charAt"](_0x31f5bb++));
						~_0x30d0ba &&
							((_0x512dd4 =
								_0x249926 % 0x4 ? _0x512dd4 * 0x40 + _0x30d0ba : _0x30d0ba),
								_0x249926++ % 0x4)
							? (_0x3ad98a += String["fromCharCode"](
								0xff & (_0x512dd4 >> ((-0x2 * _0x249926) & 0x6))
							))
							: 0x0
					) {
						_0x30d0ba = _0x24db3a["indexOf"](_0x30d0ba);
					}
					return _0x3ad98a;
				});
		})();
		function _0x1ae774(_0x4d9857, _0x7fa80a) {
			var _0x407e7d = [],
				_0x2deef8 = 0x0,
				_0x2e9dc4,
				_0x28847f = "",
				_0xfd9e24 = "";
			_0x4d9857 = atob(_0x4d9857);
			for (
				var _0x3f4807 = 0x0, _0x475002 = _0x4d9857["length"];
				_0x3f4807 < _0x475002;
				_0x3f4807++
			) {
				_0xfd9e24 +=
					"%" +
					("00" + _0x4d9857["charCodeAt"](_0x3f4807)["toString"](0x10))[
						"slice"
					](-0x2);
			}
			_0x4d9857 = decodeURIComponent(_0xfd9e24);
			for (var _0x24ad84 = 0x0; _0x24ad84 < 0x100; _0x24ad84++) {
				_0x407e7d[_0x24ad84] = _0x24ad84;
			}
			for (_0x24ad84 = 0x0; _0x24ad84 < 0x100; _0x24ad84++) {
				_0x2deef8 =
					(_0x2deef8 +
						_0x407e7d[_0x24ad84] +
						_0x7fa80a["charCodeAt"](_0x24ad84 % _0x7fa80a["length"])) %
					0x100;
				_0x2e9dc4 = _0x407e7d[_0x24ad84];
				_0x407e7d[_0x24ad84] = _0x407e7d[_0x2deef8];
				_0x407e7d[_0x2deef8] = _0x2e9dc4;
			}
			_0x24ad84 = 0x0;
			_0x2deef8 = 0x0;
			for (var _0x431598 = 0x0; _0x431598 < _0x4d9857["length"]; _0x431598++) {
				_0x24ad84 = (_0x24ad84 + 0x1) % 0x100;
				_0x2deef8 = (_0x2deef8 + _0x407e7d[_0x24ad84]) % 0x100;
				_0x2e9dc4 = _0x407e7d[_0x24ad84];
				_0x407e7d[_0x24ad84] = _0x407e7d[_0x2deef8];
				_0x407e7d[_0x2deef8] = _0x2e9dc4;
				_0x28847f += String["fromCharCode"](
					_0x4d9857["charCodeAt"](_0x431598) ^
					_0x407e7d[(_0x407e7d[_0x24ad84] + _0x407e7d[_0x2deef8]) % 0x100]
				);
			}
			return _0x28847f;
		}
		_0xa18f["wUfDiR"] = _0x1ae774;
		_0xa18f["GvQaSQ"] = {};
		_0xa18f["uAxCPA"] = !![];
	}
	var _0x1c2372 = _0xa18f["GvQaSQ"][_0x451302];
	if (_0x1c2372 === undefined) {
		if (_0xa18f["NSYJSy"] === undefined) {
			_0xa18f["NSYJSy"] = !![];
		}
		_0x14fcac = _0xa18f["wUfDiR"](_0x14fcac, _0x7fa80a);
		_0xa18f["GvQaSQ"][_0x451302] = _0x14fcac;
	} else {
		_0x14fcac = _0x1c2372;
	}
	return _0x14fcac;
...
async function requestAlgo() {
	var _0x464b07 = {
		uLhHz: function (_0x39233a, _0xadee72) {
			return _0x39233a + _0xadee72;
		},
		hHsnH: _0xa18f("‮0", "NL4*"),
		CAVAV: _0xa18f("‮1", "eQqP"),
		VlxDE: _0xa18f("‮2", "jA5)"),
		HCHhg: function (_0x408edb, _0x546372, _0x56378f) {
			return _0x408edb(_0x546372, _0x56378f);
		},
		ihNIw: "3.0",
		UcktU: function (_0x1ce2f2, _0x7c228) {
			return _0x1ce2f2 !== _0x7c228;
		},
		jnCgi: "HQZWp",
		LFFgu: _0xa18f("‮3", "72ZM"),
		bvcXd: function (_0xef261c) {
			return _0xef261c();
		},
		ySAEB: _0xa18f("‫4", "gN4R"),
		VpeuF: function (_0x422540, _0x301ad4) {
			return _0x422540(_0x301ad4);
		},
		vDYYj: function (_0x2b7d39, _0x1fdac1) {
			return _0x2b7d39 == _0x1fdac1;
		},
		DyiiW: function (_0x1a3e39, _0x47d2b2) {
			return _0x1a3e39 < _0x47d2b2;
		},
		gOSTc: function (_0x294f1c, _0x389ea5) {
			return _0x294f1c + _0x389ea5;
		},
		Kdmok: function (_0x15f812, _0x30f70f) {
			return _0x15f812(_0x30f70f);
		},
		mCwTG: function (_0x34edf8, _0x3f7775) {
			return _0x34edf8 - _0x3f7775;
		},
		xWfTq: function (_0x373def, _0x5a2a5e) {
			return _0x373def + _0x5a2a5e;
		},
		fIqcr: function (_0x326029, _0x4f37dd) {
			return _0x326029 + _0x4f37dd;
		},
		rcBCy: function (_0x49de94, _0x19dbe8) {
			return _0x49de94 * _0x19dbe8;
		},
		oYKOL: function (_0x2df178, _0x59b6ff) {
			return _0x2df178 - _0x59b6ff;
		},
		iLyCd: function (_0x143a8f, _0x49d5ad) {
			return _0x143a8f - _0x49d5ad;
		},
		APxTa: function (_0x8c947e, _0x4c5ed5) {
			return _0x8c947e - _0x4c5ed5;
		},
		pdDvj: "application/json",
		cDOxU: _0xa18f("‮5", "T0c7"),
		Djfoo: "https://prodev.m.jd.com",
		IKOyD: _0xa18f("‫6", "Pypn"),
	};
	var _0x5a44ab = "",
		_0xd24bfd = _0x464b07[_0xa18f("‮7", "eQqP")],
		_0x25abb5 = _0xd24bfd,
		_0x212b2a = (Math["random"]() * 0xa) | 0x0;
	do {
		ss = _0x464b07[_0xa18f("‮8", "eQqP")](
			_0x464b07[_0xa18f("‮9", "5HoE")](getRandomIDPro, {
				size: 0x1,
				customDict: _0xd24bfd,
			}),
			""
		);
		if (_0x464b07["vDYYj"](_0x5a44ab["indexOf"](ss), -0x1)) _0x5a44ab += ss;
	} while (_0x464b07["DyiiW"](_0x5a44ab["length"], 0x3));
	for (let _0x215b16 of _0x5a44ab["slice"]())
		_0x25abb5 = _0x25abb5[_0xa18f("‫a", "kR(b")](_0x215b16, "");
	$["fp"] =
		_0x464b07["gOSTc"](
			getRandomIDPro({ size: _0x212b2a, customDict: _0x25abb5 }),
			""
		) +
		_0x5a44ab +
		_0x464b07[_0xa18f("‫b", "H%d5")](getRandomIDPro, {
			size: _0x464b07[_0xa18f("‮c", "sGIs")](
				_0x464b07[_0xa18f("‫d", "EO)X")](
					0xe,
					_0x464b07["xWfTq"](_0x212b2a, 0x3)
				),
				0x1
			),
			customDict: _0x25abb5,
		}) +
		_0x212b2a +
		"";
	$["fp"] = _0x464b07["xWfTq"](
		_0x464b07[_0xa18f("‫e", "$wA^")](
			_0x464b07[_0xa18f("‮f", "yZ^0")](
				_0x464b07["fIqcr"](
					_0x464b07[_0xa18f("‫10", "fYXW")](
						"0",
						_0x464b07[_0xa18f("‫11", "5qKN")](
							_0x464b07[_0xa18f("‫12", "08SP")](0x7c7, 0x2a),
							""
						)
					),
					_0x464b07[_0xa18f("‮13", "cY(O")](0x3, 0x9)
				),
				_0x464b07[_0xa18f("‮14", "5qKN")](
					_0x464b07["rcBCy"](
						_0x464b07[_0xa18f("‮15", "tGEx")](0x1ff, 0x1b9),
						0xb
					),
					0x2bc
				)
			),
			_0x464b07[_0xa18f("‮16", "GuL0")](
				_0x464b07[_0xa18f("‫17", "P5OF")](0x1c9, 0x2f7),
				0x53c8f
			)
		),
		0x77
	...
	let _0x1ec956 = {
		url: _0xa18f("‮18", "3#FG"),
		headers: {
			Accept: _0x464b07[_0xa18f("‮19", "6&7h")],
			"Content-Type": "application/json",
			"Accept-Encoding": _0xa18f("‫1a", "GuL0"),
			"Accept-Language": _0x464b07[_0xa18f("‫1b", "5qKN")],
			Origin: _0x464b07["Djfoo"],
			Referer: _0x464b07[_0xa18f("‫1c", "ria#")],
			"User-Agent": $["UA"],
		},
		body:
			"{\x22version\x22:\x223.0\x22,\x22fp\x22:\x22" +
			$["fp"] +
			_0xa18f("‫1d", "6&7h") +
			Date[_0xa18f("‮1e", "GuL0")]() +
			_0xa18f("‮1f", "kR(b"),
	};
	return new Promise(async (_0x43815a) => {
		var _0x4a1691 = {
			ItLKv: function (_0x14295e, _0x486851) {
				return _0x464b07["uLhHz"](_0x14295e, _0x486851);
			},
			xPywR: _0x464b07[_0xa18f("‫20", "(VTV")],
			FJeaB: _0x464b07[_0xa18f("‫21", "mA5g")],
			vChTw: _0xa18f("‮22", "xzG9"),
			EgLYv: _0x464b07[_0xa18f("‫23", "Pypn")],
			OrulT: function (_0x4412e5, _0x3dd008, _0x3d15c2) {
				return _0x464b07[_0xa18f("‮24", "P2[M")](
					_0x4412e5,
					_0x3dd008,
					_0x3d15c2
				);
			},
			mOHPt: _0x464b07[_0xa18f("‫25", "Pypn")],
			QRMOA: function (_0x5b73a0, _0x413534) {
				return _0x5b73a0(_0x413534);
			},
			vDJvv: function (_0x5ad268, _0x3ee1c6) {
				return _0x464b07["UcktU"](_0x5ad268, _0x3ee1c6);
			},
			YyDmg: "mRpBV",
			aQTbj: _0x464b07[_0xa18f("‮26", "08SP")],
			ilsKH: function (_0x358b0d, _0x3c7e95) {
				return _0x358b0d === _0x3c7e95;
			},
			YDIxz: _0x464b07[_0xa18f("‫27", "08SP")],
			StOmz: _0xa18f("‮28", "QPP6"),
			thUQP: function (_0x4bcd4f) {
				return _0x464b07[_0xa18f("‫29", "6&7h")](_0x4bcd4f);
			},
		};
		$[_0xa18f("‮2a", "vtbm")](_0x1ec956, (_0x333c33, _0xcf2cd8, _0x279d98) => {
			var _0xbb2fa9 = {
				vnvRx: function (_0x2cc2ca, _0x5eac80) {
					return _0x4a1691["ItLKv"](_0x2cc2ca, _0x5eac80);
				},
				pmNQE: _0x4a1691["xPywR"],
				AsaKq: _0x4a1691[_0xa18f("‮2b", "n@M6")],
				DNkxp: _0xa18f("‫2c", "P5OF"),
				wuaoX: _0x4a1691[_0xa18f("‮2d", "@b$3")],
				PkuwI: _0xa18f("‫2e", "5qKN"),
				zUGMP: _0x4a1691[_0xa18f("‮2f", "XcAv")],
				yADoy: _0xa18f("‫30", "vtbm"),
				dBbAq: function (_0x41e6dd, _0x6954fc, _0x57ce94) {
					return _0x4a1691["OrulT"](_0x41e6dd, _0x6954fc, _0x57ce94);
				},
				lPqLD: "yyyyMMddhhmmssSSS",
				lkYlm: "6a98d",
				neHMo: _0x4a1691["mOHPt"],
				ARTsO: function (_0x2480ad, _0x353910) {
					return _0x4a1691[_0xa18f("‮31", "mA5g")](_0x2480ad, _0x353910);
				},
			};
			if (
				_0x4a1691["vDJvv"](
					_0x4a1691[_0xa18f("‮32", "8*LW")],
					_0xa18f("‫33", "n@M6")
				)
			) {
				return (
					_0xbb2fa9[_0xa18f("‮34", "kR(b")](e[_0xbb2fa9["pmNQE"]], ":") +
					e[_0xa18f("‮35", "3#FG")]
				);
			} else {
				try {
					if (_0x4a1691[_0xa18f("‮36", "#$gz")] !== _0x4a1691["aQTbj"]) {
						var _0x2df12e = {
							pTzFu: function (_0x3254a0, _0x158aec) {
								return _0x3254a0 + _0x158aec;
							},
							CCkOr: _0xa18f("‮37", "eQqP"),
						};
						let _0x4c1567 = [
							{ key: _0xbb2fa9[_0xa18f("‫38", "]pyt")], value: "u" },
							{
								key: _0xbb2fa9[_0xa18f("‮39", "ria#")],
								value: $[_0xa18f("‮3a", "T0c7")]
								[_0xa18f("‮3b", "EUyG")]($["toStr"](body, body))
								[_0xa18f("‮3c", "fYXW")](),
							},
							{ key: "client", value: _0xbb2fa9[_0xa18f("‫3d", "eQqP")] },
							{
								key: _0xa18f("‫3e", "EUyG"),
								value: _0xbb2fa9[_0xa18f("‫3f", "EUyG")],
							},
							{ key: "functionId", value: _0xbb2fa9[_0xa18f("‫40", "#$gz")] },
						];
						let _0x2e2f29 = _0x4c1567[_0xa18f("‮41", "vtbm")](function (
							_0x475f29
						) {
							return _0x2df12e[_0xa18f("‮42", "8*LW")](
								_0x2df12e[_0xa18f("‫43", "Pypn")](
									_0x475f29[_0xa18f("‫44", "(VTV")],
									":"
								),
								_0x475f29[_0x2df12e[_0xa18f("‫45", "yZ^0")]]
							);
						})[_0xbb2fa9["yADoy"]]("&");
						let _0x1e1c2d = Date["now"]();
						let _0x3d8b0c = "";
						let _0x3f7adf = _0xbb2fa9["dBbAq"](
							format,
							_0xbb2fa9[_0xa18f("‮46", "eQqP")],
							_0x1e1c2d
						);
						_0x3d8b0c = $[_0xa18f("‫47", "cY(O")](
							$[_0xa18f("‮48", "EUyG")],
							$["fp"]["toString"](),
							_0x3f7adf[_0xa18f("‫49", "sGIs")](),
							_0xbb2fa9[_0xa18f("‫4a", "shJe")][_0xa18f("‫4b", "jA5)")](),
							$[_0xa18f("‮4c", "5qKN")]
						)[_0xa18f("‮4d", "$wA^")]();
						const _0x4211f9 = $["CryptoJS"]
						["HmacSHA256"](_0x2e2f29, _0x3d8b0c[_0xa18f("‮4e", "]pyt")]())
						["toString"]();
						let _0x286c0a = [
							""["concat"](_0x3f7adf[_0xa18f("‮4f", "5%j8")]()),
							""[_0xa18f("‫50", "P2[M")]($["fp"][_0xa18f("‫51", "EUyG")]()),
							""["concat"](_0xbb2fa9["lkYlm"]["toString"]()),
							""[_0xa18f("‮52", "5%j8")]($[_0xa18f("‮53", "AJkh")]),
							""["concat"](_0x4211f9),
							_0xbb2fa9[_0xa18f("‫54", "tGEx")],
							""["concat"](_0x1e1c2d),
						][_0xa18f("‮55", "3#FG")](";");
						return _0xbb2fa9[_0xa18f("‫56", "eQqP")](
							encodeURIComponent,
							_0x286c0a
						);
					} else {
						const {
							...
							msg,
							data: { result } = {},
						} = JSON[_0xa18f("‮57", "5%j8")](_0x279d98);
						$[_0xa18f("‮58", "cY(O")] = result["tk"];
						$[_0xa18f("‫59", "vtbm")] = new Function(
							_0xa18f("‮5a", "AJkh") + result["algo"]
						)();
					}
				} catch (_0x3eb0aa) {
					$[_0xa18f("‮5b", "#$gz")](_0x3eb0aa, _0xcf2cd8);
				} finally {
					if (
						_0x4a1691["ilsKH"](
							_0x4a1691[_0xa18f("‮5c", "QPP6")],
							_0x4a1691[_0xa18f("‮5d", "jA5)")]
						)
					) {
						_0x43815a();
					} else {
						_0x4a1691[_0xa18f("‮5e", "5%j8")](_0x43815a);
					}
				}
			}
		});
...
function getRandomIDPro() {
	var _0x6ed2a5 = {
		qfeDf: function (_0x2c0256, _0x41bfa9) {
			return _0x2c0256 === _0x41bfa9;
		},
		XDbGX: function (_0x167bdf, _0x1c1fa1) {
			return _0x167bdf < _0x1c1fa1;
		},
		dyNZx: function (_0x39bce1, _0xd7e06a) {
			return _0x39bce1 === _0xd7e06a;
		},
		IIEXo: _0xa18f("‫5f", "Pypn"),
		ZHuxx: "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ",
		XmqfX: _0xa18f("‮60", "3#FG"),
		WijrB: function (_0x5a5e61, _0x30f773) {
			return _0x5a5e61 | _0x30f773;
		},
		zFojn: function (_0xbd1f1f, _0x532e82) {
			return _0xbd1f1f * _0x532e82;
		},
	};
	var _0x42b29a,
		_0x46fd88,
		_0x4628b7 = _0x6ed2a5[_0xa18f("‮61", "xzG9")](
			void 0x0,
			(_0x9a0e27 = (_0x46fd88 =
				_0x6ed2a5[_0xa18f("‮62", "vtbm")](
					0x0,
					arguments[_0xa18f("‮63", "EO)X")]
			...
			? 0xa
			: _0x9a0e27,
		_0x9a0e27 = _0x6ed2a5["dyNZx"](
			void 0x0,
			(_0x9a0e27 = _0x46fd88[_0xa18f("‫65", "eQqP")])
		)
			? _0x6ed2a5[_0xa18f("‮66", "fYXW")]
			: _0x9a0e27,
		_0x2be5d2 = "";
	if (
		(_0x46fd88 = _0x46fd88["customDict"]) &&
		_0xa18f("‫67", "dY]c") == typeof _0x46fd88
	)
		_0x42b29a = _0x46fd88;
	else
		switch (_0x9a0e27) {
			case _0xa18f("‫68", "eBFO"):
				_0x42b29a = _0x6ed2a5[_0xa18f("‮69", "@b$3")];
				break;
			case _0xa18f("‫6a", "NL4*"):
				_0x42b29a = _0x6ed2a5[_0xa18f("‮6b", "Z^8V")];
				break;
			case _0x6ed2a5[_0xa18f("‮6c", "08SP")]:
			default:
				_0x42b29a = _0xa18f("‮6d", "vtbm");
		}
	for (; _0x4628b7--;)
		_0x2be5d2 +=
			_0x42b29a[
			_0x6ed2a5["WijrB"](
				_0x6ed2a5[_0xa18f("‫6e", "72ZM")](
					Math[_0xa18f("‫6f", "sGIs")](),
					_0x42b29a["length"]
				),
				0x0
			)
...
function h5stSign(_0xa20305) {
	var _0x148997 = {
		fiKAR: function (_0x58d61d, _0x44ef5c) {
			return _0x58d61d + _0x44ef5c;
		},
		PApQW: _0xa18f("‫70", "P2[M"),
		fLSif: _0xa18f("‫71", "kR(b"),
		WsAAD: _0xa18f("‮72", "AJkh"),
		xfSIu: _0xa18f("‮73", ")Aq4"),
		msvpT: "client",
		iWeye: _0xa18f("‫74", "5%j8"),
		PDhPG: _0xa18f("‮75", "EO)X"),
		Qqtip: _0xa18f("‮76", "7WKC"),
		QFpTZ: function (_0x348bf6, _0x5335c8, _0xab439) {
			return _0x348bf6(_0x5335c8, _0xab439);
		},
		ugbcy: _0xa18f("‮77", "eQqP"),
		OvqYH: _0xa18f("‫78", "6&7h"),
		VgnqC: _0xa18f("‫79", "shJe"),
		eeBvF: function (_0x3fc045, _0x1f9f9d) {
			return _0x3fc045(_0x1f9f9d);
		},
	};
	let _0x117aa5 = [
		{ key: _0x148997[_0xa18f("‮7a", "5qKN")], value: "u" },
		{
			key: _0x148997["xfSIu"],
			value: $["CryptoJS"]
			[_0xa18f("‫7b", "GuL0")](
				$[_0xa18f("‮7c", "QPP6")](_0xa20305, _0xa20305)
			)
			["toString"](),
		},
		{
			key: _0x148997[_0xa18f("‫7d", "Pypn")],
			value: _0x148997[_0xa18f("‫7e", "ria#")],
		},
		{ key: "clientVersion", value: _0x148997[_0xa18f("‮7f", "08SP")] },
		{ key: _0x148997["Qqtip"], value: _0xa18f("‮80", "gN4R") },
	];
	let _0x4e18f2 = _0x117aa5[_0xa18f("‮41", "vtbm")](function (_0xab0db1) {
		return _0x148997[_0xa18f("‫81", "Pypn")](
			_0xab0db1[_0x148997["PApQW"]] + ":",
			_0xab0db1[_0x148997[_0xa18f("‫82", "]pyt")]]
		);
	})[_0xa18f("‫83", "@b$3")]("&");
	let _0x3df1eb = Date["now"]();
	let _0x24a4d3 = "";
	let _0x1596ac = _0x148997[_0xa18f("‫84", "5qKN")](
		format,
		_0x148997[_0xa18f("‫85", "tGEx")],
		_0x3df1eb
	);
	_0x24a4d3 = $[_0xa18f("‮86", "3#FG")](
		$[_0xa18f("‫87", "T0c7")],
		$["fp"][_0xa18f("‮88", "eBFO")](),
		_0x1596ac["toString"](),
		_0xa18f("‫89", "yZ^0")[_0xa18f("‮8a", "T0c7")](),
		$[_0xa18f("‫8b", "tGEx")]
	)[_0xa18f("‮8c", "AJkh")]();
	const _0x1d22de = $["CryptoJS"]
	["HmacSHA256"](_0x4e18f2, _0x24a4d3[_0xa18f("‮4f", "5%j8")]())
	["toString"]();
	let _0x334d36 = [
		""[_0xa18f("‫8d", "q5X8")](_0x1596ac["toString"]()),
		""["concat"]($["fp"][_0xa18f("‮8e", "H%d5")]()),
		""[_0xa18f("‮8f", "tGEx")](_0x148997["OvqYH"][_0xa18f("‫4b", "jA5)")]()),
		""[_0xa18f("‫90", "$wA^")]($[_0xa18f("‫91", "Pypn")]),
		""[_0xa18f("‮92", "gN4R")](_0x1d22de),
		_0x148997[_0xa18f("‫93", "shJe")],
		""[_0xa18f("‫94", "KR7u")](_0x3df1eb),
	][_0xa18f("‮95", "P5OF")](";");
	return _0x148997[_0xa18f("‫96", "UYX2")](encodeURIComponent, _0x334d36);
}
function format(_0x119f39, _0x50bc78) {
	var _0x2cc762 = {
		ethuF: function (_0x5521cb) {
			return _0x5521cb();
		},
		gKcDZ: "bLDHM",
		mmajJ: "000",
		ierna: function (_0x32b645, _0x4e6c15) {
			return _0x32b645 == _0x4e6c15;
		},
		ERcmm: "yyyy-MM-dd",
		Dprqu: function (_0x135e11, _0x12543b) {
			return _0x135e11 + _0x12543b;
		},
		YcblL: function (_0x98fdd, _0x269443) {
			return _0x98fdd / _0x269443;
		},
		HhXUu: function (_0x1dd9c3, _0x2198a4) {
			return _0x1dd9c3 - _0x2198a4;
		},
	};
	if (!_0x119f39) _0x119f39 = _0x2cc762["ERcmm"];
	var _0x3e62a6;
	if (!_0x50bc78) {
		_0x3e62a6 = Date[_0xa18f("‫97", "xzG9")]();
	} else {
		_0x3e62a6 = new Date(_0x50bc78);
	}
	var _0x23d8cc,
		_0x2d70cc = new Date(_0x3e62a6),
		_0xff5770 = _0x119f39,
		_0x2b216d = {
			"M+": _0x2cc762[_0xa18f("‮98", "$wA^")](
				_0x2d70cc[_0xa18f("‮99", "yZ^0")](),
				0x1
			),
			"d+": _0x2d70cc[_0xa18f("‮9a", "jA5)")](),
			"D+": _0x2d70cc["getDate"](),
			"h+": _0x2d70cc[_0xa18f("‮9b", "08SP")](),
			"H+": _0x2d70cc[_0xa18f("‮9c", "tGEx")](),
			"m+": _0x2d70cc[_0xa18f("‮9d", "dY]c")](),
			"s+": _0x2d70cc["getSeconds"](),
			"w+": _0x2d70cc[_0xa18f("‫9e", "tGEx")](),
			"q+": Math[_0xa18f("‫9f", "5qKN")](
				_0x2cc762["YcblL"](_0x2d70cc[_0xa18f("‫a0", "XcAv")]() + 0x3, 0x3)
			),
			"S+": _0x2d70cc[_0xa18f("‮a1", "GuL0")](),
		};
	/(y+)/i[_0xa18f("‮a2", "72ZM")](_0xff5770) &&
		(_0xff5770 = _0xff5770["replace"](
			RegExp["$1"],
			""
			["concat"](_0x2d70cc[_0xa18f("‮a3", "shJe")]())
			[_0xa18f("‮a4", "5qKN")](
				_0x2cc762[_0xa18f("‮a5", "7WKC")](
					0x4,
					RegExp["$1"][_0xa18f("‮a6", "@b$3")]
				)
			)
		));
	Object["keys"](_0x2b216d)[_0xa18f("‮a7", "5HoE")]((_0x23d8cc) => {
		if (
			new RegExp("("["concat"](_0x23d8cc, ")"))[_0xa18f("‮a8", "mA5g")](
				_0xff5770
			)
		) {
			if (_0xa18f("‮a9", ")Aq4") === _0x2cc762["gKcDZ"]) {
				var _0x3e62a6,
					_0x119f39 =
						"S+" === _0x23d8cc ? _0x2cc762[_0xa18f("‫aa", "vtbm")] : "00";
				_0xff5770 = _0xff5770["replace"](
					RegExp["$1"],
					_0x2cc762["ierna"](0x1, RegExp["$1"][_0xa18f("‫ab", "QPP6")])
						? _0x2b216d[_0x23d8cc]
						: ""
						[_0xa18f("‮ac", "EUyG")](_0x119f39)
						[_0xa18f("‫ad", "5qKN")](_0x2b216d[_0x23d8cc])
						["substr"](
							""[_0xa18f("‮ae", "fYXW")](_0x2b216d[_0x23d8cc])[
							_0xa18f("‫af", "jA5)")
							]
						)
				);
				...
				try {
					const {
						...
						msg,
						data: { result } = {},
					} = JSON[_0xa18f("‮b0", "T0c7")](data);
					$[_0xa18f("‮b1", "]pyt")] = result["tk"];
					$["genKey"] = new Function(_0xa18f("‮b2", "T0c7") + result["algo"])();
				} catch (_0x12018a) {
					$[_0xa18f("‫b3", "5HoE")](_0x12018a, resp);
				} finally {
					_0x2cc762["ethuF"](resolve);
				}
			}
		}
	});
