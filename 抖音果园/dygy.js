/**
 * 抖音果园 
 * cron 18 7 * * *  yml2213_javascript_master/dygy.js
 * 
 * 抖音果园 微信小程序 
 * 3-27   签到任务 、 新手彩蛋  点赞 完成  有效期测试中 已稳定2天 
 * 感谢所有测试人员
 * ========= 青龙 =========
 * 变量格式：  export dygyCookies='账号1 user_id的值 @ 账号2 user_id的值'  多个账号用 @分割 
...
let UA = ($.isNode() ? process.env.jrttjsbUA : $.getdata('jrttjsbUA')) || 'User-Agent: Mozilla/5.0 (iPhone; CPU iPhone OS 12_4_1 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Mobile/15E148';
	...
	if (!(await Envs()))  //多账号分割 判断变量是否为空  初步处理多账号
		...
		for (let index = 0; index < dygyCookiesArr.length; index++) {
			...
			if (n > 18 && n < 21) {
				...
				$.log('开始 【晚餐礼包】')
				await dinner_package(ck);
				await $.wait(2 * 1000);

			} else {
				console.log(`时间段不在18-21点, 跳过晚餐礼包任务\n`);
			}
			...
			try {
				...
				if (result.status_code == 0) {
					...
					for (let value of tasksarr) {
						...
						if (value.id == 1) {
							console.log(`\n 任务状态\n\n每日免费领水滴:已完成${value.round_info.current_round}/${value.round_info.total_round} 次 `)
							msg += `\n 任务状态\n\n每日免费领水滴:已完成${value.round_info.current_round}/${value.round_info.total_round} 次 `
							$.msg(`\n 【${$.name}】\n 任务状态\n\n每日免费领水滴:已完成${value.round_info.current_round}/${value.round_info.total_round} 次`)
 ...
 */
function dinner_package(ck, timeout = 3 * 1000) {
		...
		$.get(request_url, async (error, response, data) => {
			try {
				...
				if (result.status_code == 0) {
					...
					console.log(`\n【晚餐礼包】领取成功了🎉 ，获得水滴${result.data.task.reward_item.num} 个 ， 领取后后共有 ${result.data.kettle.water_num} 个水滴 !`)
					msg += `\n【晚餐礼包】领取成功了🎉 ，获得水滴${result.data.task.reward_item.num} 个 ， 领取后后共有 ${result.data.kettle.water_num} 个水滴 !`
					$.msg(`\n【${$.name}】【晚餐礼包】领取成功了🎉 ，获得水滴${result.data.task.reward_item.num} 个 ， 领取后后共有 ${result.data.kettle.water_num} 个水滴 !`)

				} else if (result.status_code === "1001") {

					$.log(`\n【晚餐礼包】 失败 ,可能是: ${result.message}!\n `)
					msg += `\n【晚餐礼包】 失败 ,可能是: ${result.message}!\n`
					$.msg(`【${$.name}】 【晚餐礼包】: ${result.message}`)

				} else {

					$.log(`\n【晚餐礼包】 失败 ❌ 了呢,可能是网络被外星人抓走了!\n `)
					msg += `\n【晚餐礼包】 失败 ❌ 了呢,可能是网络被外星人抓走了!\n`
					$.msg(`【${$.name}】 【晚餐礼包】: 失败 ❌ 了呢,可能是网络被外星人抓走了!`)
		...
		$.get(request_url, async (error, response, data) => {
			try {
				...
				if (result.status_code == 0) {
					...
					let challengearr = result.data.red_points.challenge
