 ...
 * ========= 重写  =========
 * url:   game4JFLY/getPlayerInfo
 * 类型:   script-request-body
 * 路径:   xxxxx.js
 * 域名:   20315128-3.hdpyqe.com
 * ====================================
...
async function tips(ckArr) {
	...
	let Version = `\n📌 本地脚本: V 0.2.3  远程仓库脚本: V ${Version_latest}`
	DoubleLog(`${Version}\n📌 🆙 更新内容: ${Change}`);
	DoubleLog(`${thank}`);
	await wyy();
...
!(async () => {
	if (typeof $request !== "undefined") {  // 严格不相等
		...
		let ckArr = await Variable_Check(ckStr, "bhyhkjklb_data");
		await tips(ckArr);
...
/**
 * 重写 
 * https://20315128-3.hdpyqe.com/api/game4JFLY/getPlayerInfo?canal=-1&playerOrigin=3&uid=&extra=&aid=20315128&gameId=3&_openId=oosnVwjB4KC1mwyr_Whjo_FsG79c
 */
async function GetRewrite() {
	if ($request.url.indexOf("game4JFLY/getPlayerInfo") > -1) {
		let body_data = $request.body;
		ck = body_data.match(/unionId=(\S*)&share/)[1];
		if (ckStr) {
			if (ckStr.indexOf(ck) == -1) {  // 找不到返回 -1
				ckStr = ckStr + "@" + ck;
				$.setdata(ckStr, "bhyhkjklb_data");
				ckList = ckStr.split("@");
				$.msg($.name + ` 获取第${ckList.length}个 ck 成功: ${ck}`);
			}
		} else {
			$.setdata(ck, "bhyhkjklb_data");
