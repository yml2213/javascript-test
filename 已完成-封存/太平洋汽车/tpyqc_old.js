...
let msg = '';
let ck = '';
let tpyqc_data = process.env.tpyqc_data;
/////////////////////////////////////////////////////////
...
!(async () => {
	...
	if (!(await Envs()))  //多账号分割 判断变量是否为空  初步处理多账号
		...
		console.log(`\n=================== 共找到 ${tpyqc_dataArr.length} 个账号 ===================`)
		if (debug) {
			console.log(`【debug】 这是你的账号数组:\n ${tpyqc_dataArr}`);
		}


		for (let index = 0; index < tpyqc_dataArr.length; index++) {


			let num = index + 1
			console.log(`\n========= 开始【第 ${num} 个账号】=========\n`)

			data = tpyqc_dataArr[index].split('&');
			if (debug) {
				console.log(`\n 【debug】 这是你第 ${num} 账号信息:\n ${data}\n`);
			}
...
async function Envs() {
	if (tpyqc_data) {
		if (tpyqc_data.indexOf("@") != -1) {
			tpyqc_data.split("@").forEach((item) => {
				tpyqc_dataArr.push(item);
			});
		} else {
			tpyqc_dataArr.push(tpyqc_data);
		}
	} else {
		console.log(`\n 【${$.name}】：未填写变量 tpyqc_data`)
		return;
	}
