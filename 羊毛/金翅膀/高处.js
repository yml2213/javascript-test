...
const ck_ = `372330200112234650#4650`   // 快速测试, 直接填入ck即可测试
...
class User {
    ...
    async login() {
        ...
        if (res.code === 200) {
            ...
            for (const a of res.data.apply) {
                if (a.subjectTitleFull = '高处作业/高处安装维护拆除作业/初培') {
                    console.log(a.applyid)
                    console.log(`===================`)
                    this.applyid = a.applyid
                    this.px_name = a.subjectTitleFull

                }
            }

            this.log(`${this.nickname}--${res.msg}, 培训类型:${this.px_name} -- applyid: ${this.applyid}`)
    ...
    // https://jinchibang.taijieyun.com/index/material/mslist?msid=85&applyid=14505&cate=%E7%90%86%E8%AE%BA
    async big_taskList() {
        const options = {
            method: "get",
            url: `https://jinchibang.taijieyun.com/index/material/mslist?msid=85&applyid=${this.applyid}&cate=%E7%90%86%E8%AE%BA`,
            headers: this.hd,
    ...
    // https://jinchibang.taijieyun.com/index/material/learnlog?applyid=14133&mid=14748&limit=80
    async learnlog(list) {
        const options = {
            method: "get",
            url: `https://jinchibang.taijieyun.com/index/material/learnlog?applyid=${this.applyid}&mid=${list.id}&limit=80`,
            headers: this.hd,
        }
        console.log(options)
        let { res } = await requestPromise(options)
    ...
    // {"mprogress":"1","mid":"633","msid":"61b04aea9a7d9","period":415,"done":1,"applyid":"14133","face":""}
    // {"mprogress":"0.00","mid":"633","msid":"61b04aea9a7d9","period":2,"done":0,"applyid":"14133","face":""}

    // {"mprogress":"1","mid":"633","msid":"61b04aea9a7d9","period":415,"done":1,"applyid":"14133","face":""}
    // {"mprogress":"1","mid":"641","msid":"61b04aea9a7d9","period":386,"done":1,"applyid":"14133","face":""}
    async doTask(list) {
        const options = {
            method: "post",
            url: `https://jinchibang.taijieyun.com/index/material/addlog`,
            headers: this.hd,
            json: this.encryptAES(`{"mprogress":"1","mid":"${list.id}","msid":"${list.material_subject}","period":415,"done":1,"applyid":"${this.applyid}","face":""}`)
        }
        // console.log(options)
        let { res } = await requestPromise(options)
        // console.log(JSON.stringify(res))
