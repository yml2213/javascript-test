...
class UserClass {
    ...
    async info() { // 信息
        ...
        let data = `/api/user_mumber/account_detail&&${this.SESSION}&&${reqid}&&${ts13}&&FR*r!isE5W&&57`
    ...
    async get_acw_tc() {
        ...
        let options = {
            fn: '获取acw_tc',
            method: 'get',
            url: `https://frsapnea.act.tmuact.com/answer/index.html`,
            headers: this.hd,
    ...
    async get_phpsession() { // 获取phpsession
        ...
        let options = {
            fn: '获取phpsession',
            method: 'get',
            url: `https://frsapnea.act.tmuact.com/activity/api.php?m=public&subm=front`,
            headers: {
                'User-Agent': 'Mozilla/5.0 (iPhone; CPU iPhone OS 14_2_1 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Mobile/15E148;;xsb;xsb_nanhu;1.0.2;Appstore;native_app',
                'Cookie': this.acw_tc
            }
        ...
        this.phpsession = resp['set-cookie'][0].split(';')[0]
    ...
    async check_addr() { // 检测地址
        // `https://frsapnea.act.tmuact.com/activity/api.php?m=front&subm=answer&action=quzhouregister&q=YunSLfvEi&account_id=${this.account_id}&sessionid=${this.SESSION}`,
        let options = {
            fn: '检测地址',
            method: 'get',
            url: `https://frsapnea.act.tmuact.com/activity/api.php?m=front&subm=answer&action=init&q=YunSLfvjF&isNeedLogin=true&account_id=${this.account_id}&sessionid=${this.SESSION}&need_register=2`,
            headers: this.hd,
    ...
    async init2() { // 初始化地址
        ...
        let addArr = ["大桥镇（南湖高新区）", "新丰镇", "余新镇", "凤桥镇", "湘家荡区域（七星街道）", "南湖新区（东栅街道）"]
        ...
        let options = {
            fn: '初始化地址',
            method: 'post',
            url: `https://frsapnea.act.tmuact.com/activity/api.php?m=front&subm=answer&action=nanhuregister&q=YunSLfvjF&isNeedLogin=true&account_id=${this.account_id}&sessionid=${this.SESSION}`,
            headers: this.hd,
            form: {
                'name': new_name,
                'age': "19-45岁",
    ...
    async start(num) { // 开始答题
        let options = {
            fn: '开始答题',
            method: 'get',
            url: `https://frsapnea.act.tmuact.com/activity/api.php?m=front&subm=answer&action=answerinfo&answer_id=2166&answer_start=${num}`,
            headers: this.hd,
    ...
    // https://frsapnea.act.tmuact.com/activity/api.php?m=front&subm=answer&action=answerdata&answer_id=2218
    async doSubm(num, id, win_id) { // 提交
        ...
        let options = {
            fn: '提交',
            method: 'post',
            url: `https://frsapnea.act.tmuact.com/activity/api.php?m=front&subm=answer&action=answerdata&answer_id=2166`,
            headers: this.hd,
            form: {
                'id': id,
                'option': `["${win_id}"]`,
    ...
    // https://frsapnea.act.tmuact.com/activity/api.php?m=front&subm=answer&action=answerinfo&answer_id=2218&page=10
    // https://frsapnea.act.tmuact.com/activity/api.php?m=front&subm=answer&action=answerinfo&answer_id=2218&page=2
    async doAnswer(num) { // 答题
        let options = {
            fn: '答题',
            method: 'get',
            url: `https://frsapnea.act.tmuact.com/activity/api.php?m=front&subm=answer&action=answerinfo&answer_id=2166&page=${num}`,
            headers: this.hd
    ...
    //https://frsapnea.act.tmuact.com/activity/api.php?m=front&subm=answer&action=answerresult&answer_id=2166&id=1442829

    async end(endId) { // 结束
        let options = {
            fn: '结束',
            method: 'get',
            url: `https://frsapnea.act.tmuact.com/activity/api.php?m=front&subm=answer&action=answerresult&answer_id=2166&id=${endId}`,
            headers: {
                'User-Agent': 'Mozilla/5.0 (iPhone; CPU iPhone OS 14_2_1 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Mobile/15E148;;xsb;xsb_nanhu;1.0.2;Appstore;native_app',
                'Cookie': `${this.phpsession};acw_tc=${this.account_id};`
            }
        }
        // console.log(options)
        let resp = await $.request(options)
    ...
    // https://fhda1v6s.act.tmuact.com/activity/api.php?m=front&subm=lottery&action=drawprize&preview=0&lat=&lng=
    async Lottery() { // 抽奖
        let acw_tc_lottery = await this.get_acw_tc_lottery()
        let ts13 = $.ts(13)
        ...
        let options = {
            ...
            headers: {
                'User-Agent': 'Mozilla/5.0 (iPhone; CPU iPhone OS 14_2_1 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Mobile/15E148;;xsb;xsb_nanhu;1.0.2;Appstore;native_app',
                'Cookie': `${this.phpsession}; ${acw_tc_lottery};`
            }
        }
        console.log(options)
        let resp = await $.request(options)
    ...
    async get_acw_tc_lottery() {  // 获取 acw_tc_lottery  --抽奖的

        let options = {
            fn: 'acw_tc_lottery',
            method: 'get',
            url: `https://fhda1v6s.act.tmuact.com/lottery/index.html`,
            headers: this.hd,
        ...
        this.acw_tc_lottery = resp['set-cookie'][0].split(';')[0]
        return this.acw_tc_lottery
