...
-------------------  青龙-配置文件-复制区域  -------------------
# 爱用商城
export qqzbt=" authCode @ authCode "

抓 https://ad.zyxdit.com/api/alipay/authCode  的 body 的 authCode 参数
多账号用 换行 或 @ 分割
...
class User {
    ...
    async authCode() {
        const options = {
            method: "post",
            url: `https://ad.zyxdit.com/api/alipay/authCode`,
            headers: {
        ...
        try {
            if (result.code == 0) {
                ...
                // fs.writeFile(`${ckFile}`, this.token, (err, data) => {
                //     if (err) throw err
                // })
