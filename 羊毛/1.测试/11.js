...
//console.log(`列表 ${this.bb} ${this.bn}    `)
    ...
    async fx() {
        ...
        try {
            ...
            let body = `{"safeEnc":${time8 - 20220000},"activity":"","tel":"${this.ck[0]}","id":"100","source":"APP","memberId":"${this.memberComplexCode}"}`
...
`)
        ...
        getdata(b) {
            let a = this.getval(b)
            if (/^@/.test(b)) {
                let [, c, f] = /^@(.*?)\.(.*?)$/.exec(b), d = c ? this.getval(c) : ""
                if (d) try {
                    let e = JSON.parse(d)
        ...
        setdata(c, d) {
            let a = !1
            if (/^@/.test(d)) {
                let [, b, e] = /^@(.*?)\.(.*?)$/.exec(d), f = this.getval(b),
                    i = b ? "null" === f ? null : f || "{}" : "{}"
                try {
                    let g = JSON.parse(i)
        ...
        send(b, a, f = () => {
            ...
            if ("get" == b && a.headers ? (delete a.headers["Content-Type"], delete a.headers["Content-Length"]) : a.body && a.headers && (a.headers["Content-Type"] || (a.headers["Content-Type"] = "application/x-www-form-urlencoded")), this.isSurge() || this.isLoon()) {
                this.isSurge() && this.isNeedRewrite && (a.headers = a.headers || {}, Object.assign(a.headers, {"X-Surge-Skip-Scripting": !1}))
                let c = {method: b, url: a.url, headers: a.headers, timeout: a.timeout, data: a.body}
                "get" == b && delete c.data, $axios(c).then(a => {
                    let {status: b, request: c, headers: d, data: e} = a
                    f(null, c, {statusCode: b, headers: d, body: e})
                }).catch(a => console.log(a))
            } else if (this.isQuanX()) a.method = b.toUpperCase(), this.isNeedRewrite && (a.opts = a.opts || {}, Object.assign(a.opts, {hints: !1})), $task.fetch(a).then(a => {
                ...
                let {url: d, ...e} = a
                this.instance = this.got.extend({followRedirect: !1}), this.instance[b](d, e).then(a => {
                    let {statusCode: b, request: c, headers: d, body: e} = a
                    f(null, c, {statusCode: b, headers: d, body: e})
                }, b => {
        ...
        async showmsg() {
            ...
            if ($.isNode()) {
                ...
                console.log("\n============== \u63A8\u9001 =============="), await b.sendNotify(this.name, a)
            } else this.msg(a)
        }
        ...
        msg(d = t, a = "", b = "", e) {
            let f = a => {
                if (!a) return a
                if ("string" == typeof a) return this.isLoon() ? a : this.isQuanX() ? {"open-url": a} : this.isSurge() ? {url: a} : void 0
                if ("object" == typeof a) {
                    if (this.isLoon()) {
                    ...
                    if (this.isQuanX()) {
                        let d = a["open-url"] || a.url || a.openUrl, e = a["media-url"] || a.mediaUrl
                        return {"open-url": d, "media-url": e}
                    }
                    if (this.isSurge()) return {url: a.url || a.openUrl || a["open-url"]}
                }
            }
        ...
        padStr(e, b, f = "0") {
            let a = String(e), g = b > a.length ? b - a.length : 0, c = ""
        ...
        str2json(e, f = !1) {
            ...
            for (let a of e.split("#")) {
                ...
                let b = a.indexOf("=")
                if (-1 == b) continue
                let g = a.substr(0, b), c = a.substr(b + 1)
        ...
        wait(a) {
            ...
            let b = (new Date).getTime(), c = (b - this.startTime) / 1e3
            console.log(`
