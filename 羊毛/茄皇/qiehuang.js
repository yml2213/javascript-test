...
-------------------  青龙-配置文件-复制区域  -------------------
# 茄皇 
export qiehuang=" thirdId # wid @  thirdId # wid "  
...
class UserClass {
    ...
    async userInfo() {
        ...
        if (resp.code == 0) {
            ...
            $.log(`${this.idx}: 昵称：${this.nickName}, 阳光：${this.sunshine}, 番茄：${this.tomatoNum}`)
    ...
    async taskList() {      // 任务列表
        ...
        if (resp.code == 0) {
            for (let task of resp.data) {
                if (task.taskId == 2 || task.taskId == 3) {  //2--邀请好友助力  3--邀请新人助力
                    continue
                } else {
                    switch (task.status) {
                        case 0:
                            console.log(`任务 [${task.title}] -- 未完成`)
                            let num = task.progress - task.currentProgress
                            for (let i = 0; i < num; i++) {
    ...
    // https://qiehuang-apig.xiaoyisz.com/qiehuangsecond/ga/task/doTask?id=1682271525821771776
    async doTask(task) {    // 做任务
        let options = {
            fn: '做任务',
            method: 'get',
            url: `https://qiehuang-apig.xiaoyisz.com/qiehuangsecond/ga/task/doTask?id=${task.id}`,
            headers: this.hd,
        ...
        if (resp.code == 0 && resp.data == true) {
            ...
            $.log(`${this.idx}: 昵称：${this.nickName}, ${task.title} -- ok`)
            await this.reward(task)

        } else console.log(`${options.fn}: 失败,  ${JSON.stringify(resp)}`)
    }


    // https://qiehuang-apig.xiaoyisz.com/qiehuangsecond/ga/task/reward?id=1682271525821771776
    async reward(task) {    // 领取奖励
        let options = {
            fn: '领取奖励',
            method: 'get',
        ...
        if (resp.code == 0) {
            ...
            $.log(`${this.idx}: 昵称：${this.nickName}, 任务 [${task.title}] -- 领取成功`)
