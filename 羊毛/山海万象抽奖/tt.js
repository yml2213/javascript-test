...
class User {
	...
	async userTask() {``
		...
		for (let i = 1; i < 1596; i++) {
			await this.get_answer(i)
			console.log(i)
	...
	async check_num() {
		try {
			...
			// this.log(res)
			if (res.code == 0) {
				this.log(`你当前 ${res.data.level}场, 剩余 ${res.data.number} 次`)
	...
	async start_qs(mid) {
		try {
			...
			if (res.code == 0) {
				...
				let as = await this.get_answer(res.data.id)
				console.log(as)
				await wait(5, 8)
	...
	async lottery(mid) {
		try {
			...
			// this.log(res)
			if (res.code == 0) {
				this.log(`恭喜获得 ${res.data.type} 现金 --  ${res.data.money} 元, 去提现了鸭!`)
				await this.cash(mid)
			} else if (res.code == 1) {
				this.log(res.msg)
			} else {
