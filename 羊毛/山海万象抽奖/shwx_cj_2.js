...
-------------------  青龙-配置文件-复制区域  -------------------
# 山海万象抽奖
export shwx_cj=" PHPSESSID  @ PHPSESSID "

  
抓 https://fhqpfv1n.act.tmuact.com/activity/api.php? 的 PHPSESSID参数
多账号用 换行 或 @ 分割

tg频道: https://t.me/yml2213_tg
*/

const CodeName = "山海万象抽奖"
const env = "shwx_cj"
const envSplit = ["\n", "&", "@"]
const fs = require("fs")
...
let sendLog = []
const mode = 2    // 并发-2   顺序-1
const runMax = 3  // 最大并发数量
const ckFile = `${env}.txt`
//====================================================================================================
const ck_ = `0804#fd92eee191e34ca11bc426395342b6e8
2213#fca182bc57b77e6f3a0fa72144b51a34`
...
class User {
    ...
    async cj() {
        try {
            let lat = 29.48 - randomInt(0.2, 0.5)
            let lng = 121.87 - randomInt(0.2, 0.5)
            const options = {
                method: "get",
                url: `https://fhqpfv1n.act.tmuact.com/activity/api.php?m=front&subm=lottery&action=drawprize&preview=0&lat=${lat}&lng=${lng}`,
                headers: {
                    Host: 'fhqpfv1n.act.tmuact.com',
                    'sec-ch-ua': '"Android WebView";v="119", "Chromium";v="119", "Not?A_Brand";v="24"',
                    Accept: 'application/json, text/plain, */*',
                    'sec-ch-ua-mobile': '?1',
                    'User-Agent': 'Mozilla/5.0 (Linux; Android 12; M2102J2SC Build/SKQ1.211006.001; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/119.0.6045.194 Mobile Safari/537.36;xsb_xiangshan;xsb_xiangshan;4.5.0;native_app',
                    'sec-ch-ua-platform': '"Android"',
                    'X-Requested-With': 'com.aheading.news.xiangshanrb',
                    'Sec-Fetch-Site': 'same-origin',
                    'Sec-Fetch-Mode': 'cors',
                    'Sec-Fetch-Dest': 'empty',
                    Referer: 'https://fhqpfv1n.act.tmuact.com/lottery/index.html',
            ...
            // this.log(res)
            if (res.code == 3001 && res.data.prize_id == 8188) {
                this.log(`恭喜狗子, 现金: ${res.data.credit} 元`, 1)

            } else if (res.code == 3001 && res.data.prize_id == 0) {
                this.log(`积分-20, 哈哈哈哈哈`)
                await wait()
                await this.cj()
            } else {
                this.log(res)
    ...
    async sign_in() {  // 签到
        try {
            const options = {
                method: "post",
                url: `https://shwx_cj.jiiimo.cn/api/user/agree`,
                headers: this.hd,
                form: {
                    uuid: this.uuid,
                    sign: this.sign,
    ...
    async check_num() {
        try {
            const options = {
                method: "post",
                url: `https://shwx_cj.jiiimo.cn/api/user/rule`,
                headers: this.hd,
                form: {
                    uuid: this.uuid,
                    sign: this.sign,
    ...
    async check_mid() {   // 获取博物馆 id
        try {
            const options = {
                method: "post",
                url: `https://shwx_cj.jiiimo.cn/api/museum/list`,
                headers: this.hd,
                form: {
                    uuid: this.uuid,
                    sign: this.sign,
    ...
    async start_qs(mid) {
        try {
            const options = {
                method: "post",
                url: `https://shwx_cj.jiiimo.cn/api/question/get`,
                headers: this.hd,
                form: {
                    mid: mid,
    ...
    async qs_post(mid, id, as) {
        try {
            const options = {
                method: "post",
                url: `https://shwx_cj.jiiimo.cn/api/question/post`,
                headers: this.hd,
                form: {
                    id: id,
                    select: as,
    ...
    async lottery(mid) {
        try {
            const options = {
                method: "post",
                url: `https://shwx_cj.jiiimo.cn/api/prize/get`,
                headers: this.hd,
                form: {
                    mid: mid,
                    uuid: this.uuid,
    ...
    async cash() {
        try {
            const options = {
                method: "post",
                url: `https://shwx_cj.jiiimo.cn/api/user/tixian`,
                headers: this.hd,
                form: {
                    // mid: mid,
                    uuid: this.uuid,
