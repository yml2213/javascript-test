...
(async () => {
    const { sender: s, Bucket, Adapter, sleep, utils: { parseCQText }, } = require("sillygirl")

    let ck_ = await s.param("参数2")
    ...
    class User {
        ...
        async check() {
            const options = {
                method: "get",
                url: `https://qdxsxx.119.gov.cn/alipay/mini/api/users/activeScore`,
                headers: {
                    "Host": "qdxsxx.119.gov.cn",
            ...
            if (res.code === 1001) {
                ...
                await s.reply(`您当前积分: ${res.result}`)
        ...
        async doTask1() {
            ...
            const options = {
                method: "get",
                url: `https://qdxsxx.119.gov.cn/alipay/mini/api/home/taskScord/completeTask?parameter1=${n}&parameter2=HTWKT&taskCode=CTWLAVTIME`,
                headers: this.hd,
            }
            let { res } = await requestPromise(options)
            // console.log(JSON.stringify(res))
        ...
        async doTask2() {
            ...
            const options = {
                method: "get",
                url: `https://qdxsxx.119.gov.cn/alipay/mini/api/home/taskScord/completeTask?parameter1=${n}&parameter2=HZAL&taskCode=CTWLAV`,
                headers: this.hd,
            }
            let { res } = await requestPromise(options)
            // console.log(JSON.stringify(res))
