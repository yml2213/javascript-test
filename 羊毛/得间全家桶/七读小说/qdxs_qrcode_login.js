...
-------------------  青龙-配置文件-复制区域  -------------------
# 七读小说
export qdxs=" 备注 # sessionId @ 备注 # sessionId "
...
class User {
    ...
    async main_login() {
        try {
            let utdid = `ZM${randomString(22)}`  // 生成随机设备utdid`
            let a = await this.get_issuedId(utdid)
            let _name = a[0]
            let zyeId = a[1]
            // console.log(utdid, _name, zyeId)
            await this.WX_login(utdid, _name, zyeId)
        } catch (error) {
            // console.log(error)
            console.log(error)
        }
