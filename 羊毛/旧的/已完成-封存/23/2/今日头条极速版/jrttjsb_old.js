...
/**
 * 用户信息    GET  https://api5-normal-lf.toutiaoapi.com/luckycat/lite/v1/task/page_data/?aid=35
 */
async function user_info(doTask) {
    ...
    let resp = await httpRequest(options, `用户信息`)

    if (resp.err_no == 0) {
        if (!resp.data.treasure) {
            console.log(`    用户${jrttjsb_num} 查询状态失败,CK失效 `)
            return ck_status = 1
        }
        if (doTask == 0) {
            console.log(`    账户信息:金币:${resp.data.user_income.score_balance} ,现金:${resp.data.user_income.cash_balance / 100}元 `)
        } else {
            ...
            if (resp.data.signin_detail.today_signed == false) {
                await SignIn()
            } else {
        ...
        console.log(`    用户信息: 失败 ❌ 了呢,原因未知！  ${resp} `)
        msg = `\n    用户信息: 失败 ❌ 了呢,原因未知!`
        message = `\n    用户信息: 失败 ❌ 了呢,原因未知!`
...
async function OpenTreasureBox() {
    ...
    let resp = await httpRequest(options, `开宝箱`)

    if (resp.err_no == 0) {
        console.log(`    开宝箱: 获得 ${resp.data.score_amount} 金币`)
        msg += `\n    开宝箱: 获得 ${resp.data.score_amount} 金币`
    } else {
        console.log(`    开宝箱: 失败 ❌ 了呢,原因未知！ ${resp.err_tips} `)
        msg += `\n    开宝箱: 失败 ❌ 了呢,原因未知!`
    }
}
...
async function SignIn() {
    ...
    let resp = await httpRequest(options, `签到`)

    if (resp.err_no == 0) {
        console.log(`    签到: ${resp.err_tips} ,获得 ${resp.data.score_amount} 金币,已连续签到 ${resp.data.sign_times} 天`)
        msg += `\n    签到: ${resp.err_tips} ,获得 ${resp.data.score_amount} 金币,已连续签到 ${resp.data.sign_times} 天`
    } else {
        console.log(`    签到: 失败 ❌ 了呢,原因未知！  ${resp} `)
        msg += `\n    签到: 失败 ❌ 了呢,原因未知!`
    }
}
...
async function QuerySleepStatus() {
    ...
    let resp = await httpRequest(options, `睡觉状态`)

    if (resp.err_no == 0) {
        let sleepHour = Math.floor(resp.data.sleep_last_time / 36) / 100
        if (resp.data.sleeping == true) {
            if (sleepHour >= 12) {
                await SleepStop()
            } else if (resp.data.sleep_unexchanged_score == resp.data.max_coin && curHour >= 7) {
                let rnd = Math.random()
                if (rnd > 0.95) {
                    await SleepStop()
                } else {
                    console.log(`     用户${jrttjsb_num}随机醒来时间,本次不进行醒来,已经睡了${sleepHour}小时,可以获得${resp.data.sleep_unexchanged_score}金币`)
                    msg += `\n     用户${jrttjsb_num}随机醒来时间,本次不进行醒来,已经睡了${sleepHour}小时,可以获得${resp.data.sleep_unexchanged_score}金币`
                }
            } else {
                console.log(`     用户${jrttjsb_num}睡眠中,已经睡了${sleepHour}小时,可以获得${resp.data.sleep_unexchanged_score}金币,上限${resp.data.max_coin}金币`)
                msg += `\n     用户${jrttjsb_num}睡眠中,已经睡了${sleepHour}小时,可以获得${resp.data.sleep_unexchanged_score}金币,上限${resp.data.max_coin}金币`
            }
        } else {
            if (resp.data.history_amount > 0) {
                await SleepDone(resp.data.history_amount)
            }
        ...
        console.log(`     用户${jrttjsb_num}查询睡觉状态失败：${resp.err_tips}`)
        msg += `\n     用户${jrttjsb_num}随机睡眠时间,本次不进行睡眠`
    }
}
...
async function SleepStop() {
    ...
    let resp = await httpRequest(options, `睡觉醒来`)

    if (resp.err_no == 0) {
        let sleepHour = resp.data.sleep_last_time / 3600
        console.log(`     用户${jrttjsb_num}结束睡眠,本次睡了${sleepHour}小时,可以领取${resp.data.history_amount}金币`)
        msg += `\n     用户${jrttjsb_num}结束睡眠,本次睡了${sleepHour}小时,可以领取${resp.data.history_amount}金币`
        await SleepDone(resp.data.history_amount)
    } else {
        console.log(`    睡觉醒来: 失败 ❌ 了呢,原因未知！  ${resp} `)
        msg += `\n     睡觉醒来: 失败 ❌ 了呢,原因未知!`
    }
}
...
async function SleepDone(amount) {
    ...
    let resp = await httpRequest(options, `睡觉醒来收金币`)
...
async function SleepStart() {
    ...
    let resp = await httpRequest(options, `开始睡觉`)
...
async function QueryWalkInfo() {
    ...
    let resp = await httpRequest(options, `查询走路状态`)

    if (resp.err_no == 0) {
        if (resp.data.can_get_amount > 0) await GetWalkBonus()
...
async function GetWalkBonus() {
    ...
    let resp = await httpRequest(options, `走路奖励`)

    if (resp.err_no == 0) {
        console.log(`    领取走路奖励获得 ${resp.data.score_amount} 金币`)
        msg += `\n     领取走路奖励获得 ${resp.data.score_amount} 金币`
    } else if (resp.err_no == 8005028) {
        console.log(`      ${resp.err_tips} `)
        msg += `\n      ${resp.err_tips} `
    } else {
...
async function EatInfo(eat_name, taskId) {
    ...
    let resp = await httpRequest(options, `${eat_name}`)

    if (resp.err_no == 0) {
        if (resp.data.complete_status[taskId] == false) {
            await DoneEat()
        } else {
...
async function DoneEat() {
    ...
    let resp = await httpRequest(options, `吃饭补贴`)

    if (resp.err_no == 0) {
        console.log(`     领取吃饭补贴获得 ${resp.data.score_amount} 金币`)
        msg += `\n     领取吃饭补贴获得 ${resp.data.score_amount} 金币`
    } else if (resp.err_no == 1071) {
        console.log(`     领取吃饭补贴 ${resp.err_tips}`)
        msg += `\n     领取吃饭补贴获得 ${resp.err_tips}`
    } else {
...
async function ExcitationAd(task_id) {
    ...
    let resp = await httpRequest(options, `宝箱视频奖励`)

    if (resp.err_no == 0) {
        console.log(`    领取宝箱视频奖励获得 ${resp.data.score_amount} 金币`)
        msg += `\n    领取宝箱视频奖励获得 ${resp.data.score_amount} 金币`
    } else if (resp.err_no == 1071) {
        console.log(`    领取宝箱视频奖励 ${resp.err_tips}`)
        // msg += `\n    领取宝箱视频奖励获得 ${resp.err_tips}`;
    } else {
...
async function ReadArtsReward() {
    ...
    let resp = await httpRequest(options, `阅读文章奖励`)

    if (resp.err_no == 0) {
        console.log(`    阅读文章奖励: 获得${resp.data.score_amount}金币，今日阅读总收入：${resp.data.total_score_amount}金币`)
        msg += `\n    阅读文章奖励: 获得${resp.data.score_amount}金币，今日阅读总收入：${resp.data.total_score_amount}金币`
        console.log('    等待15秒阅读下一篇...')
        await $.wait(15 * 1000)
    } else if (resp.err_no == 9) {
        console.log(`    阅读文章奖励: ${resp.err_tips}`)
        msg += `\n    阅读文章奖励: ${resp.err_tips}`
...
async function ReadDouble() {
    ...
    let resp = await httpRequest(options, `阅读翻倍`)
