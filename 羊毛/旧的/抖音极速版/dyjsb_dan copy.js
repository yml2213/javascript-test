...
nowMin = Math.round(new Date().getMinutes()).toString()
usid = 0
Notify = 0
...
class Bar {
    ...
    async userinfo() {
        ...
        if (datas.err_no == 0) {
            console.log(`${this.f}登录成功,现有${datas.data.income_data.amount1}金币,现金余额：${datas.data.income_data.amount2 / 100}元`)
            this.message += `${this.f}登录成功,现有${datas.data.income_data.amount1}金币,现金余额：${datas.data.income_data.amount2 / 100}元`
            this.logs = true
        } else {
            this.log = false
        }
    ...
    async signin() {
        ...
        if (datas.err_no == 0) {
            console.log(`${this.f}签到获得：${datas.data.amount}金币`)
    ...
    async signinvideo() {
        ...
        if (datas.err_no == 10009) {
            await this.signinvideo()
        } else if (datas.err_no == 10002) {
            await this.signinvideo()
        } else if (datas.err_no == 0) {
            console.log(`${this.f}观看签到视频获得：${datas.data.amount}金币`)
        } else {
    ...
    async advideo() {
        ...
        //  console.log(this.f,datas)
        if (datas.err_tips == '今日看广告领金币已达到上限，休息一下~明天再来看吧') {
        } else if (datas.err_tips == '很遗憾，奖励溜走了') {
            await this.advideo()
        } else if (datas.err_no == 0) {
            console.log(`${this.f}观看广告视频获得：${datas.data.amount}金币`)
        } else if (datas.err_no != 0) {
            console.log(datas)
            await this.advideo()
        }
    ...
    async gjvideostar() {
        ...
        console.log(`${this.f} 开始逛街,随机等待 ${aa / 1000} 秒`)
        await $.wait(aa)
        await this.gjvideo()
    }
    async gjvideo() {
        ...
        if (datas.err_no == 10009) {
            ...
            console.log(`${this.f}完成逛街任务：${datas.data.reward_text}`)
        } else {
    ...
    async upstep() {
        ...
        if (datas.err_no == 0) {
            console.log(`${this.f}上传步数成功！开始领取奖励`)
            await this.goway()
        } else {
            console.log(`${this.f}上传步数失败`)
        }
    }
    // 走路任务
    async goway() {
        ...
        // console.log(datas)
        if (datas.err_no == 10009 || 10002) {
            // await this.goway()
            // console.log(datas)

            if (datas.err_tips == '领取失败，请稍后再试') {
                // console.log(datas)
                console.log(`${this.f} 走路任务: ${datas.err_tips}`)
            } else {
                await this.goway()
            }

        } else if (datas.err_no == 0) {
            console.log(`${this.f}完成走路任务获得：${datas.data.reward_amount}金币`)
        }
    }
    //开宝箱
    async openbox() {
        ...
        if (datas.err_tips == '今日看广告领金币已达到上限，休息一下~明天再来看吧') {
            ...
            await this.openbox()
        } else if (datas.err_tips == '您已经开过宝箱了喔，请稍后再试') {
            console.log(`${this.f} 您已经开过宝箱了喔，请稍后再试`)
        } else if (datas.err_no == 0) {
            this.score = datas.data.excitation_ad_info.score_amount
            this.reqid = datas.data.excitation_ad_info.req_id
            this.adid = datas.data.excitation_ad_info.ad_id
            console.log(`${this.f}开宝箱获得：${datas.data.amount}金币`)
            await this.boxvideo()
        } else if (datas.err_no != 0) {
            await this.openbox()
        }
    ...
    async boxvideo() {
        ...
        // console.log(this.f,datas)
        if (datas.err_tips == '今日看广告领金币已达到上限，休息一下~明天再来看吧') {
        } else if (datas.err_tips == '很遗憾，金币溜走了～') {
            await this.boxvideo()
        } else if (datas.err_tips == '服务异常，请稍后重试') {
            await this.boxvideo()
        } else if (datas.err_tips == '广告视频准备中，请稍后再试') {
            await this.boxvideo()
        } else if (datas.err_no == 0) {
            console.log(`${this.f}看开宝箱视频获得：${datas.data.amount}金币`)
    ...
    // 吃饭领金币
    async eatmeals() {
        if (nowhour == 8 && nowMin < 30) {
            this.mealid = 0
        } else if (nowhour == 11 && nowMin < 30) {
            this.mealid = 1
        } else if (nowhour == 18 && nowMin < 30) {
            this.mealid = 2
        } else if (nowhour == 21 && nowMin < 30) {
            this.mealid = 3
        }
        let headers = {
            "Content-Type": "application/json",
        ...
        if (datas.code == 10009) {
            // 吃饭领金币
            console.log(`${this.f} 吃饭领金币 失败，即将重试`)
            eat_num++
            if (eat_num <= 5) {
                await this.eatmeals()
            }
            ...
            console.log(`${this.f}领取吃饭奖励获得：${datas.data.reward}金币`)
    ...
    async eatmealsvideofb() {
        ...
        if (datas.err_no == 10009) {
            ...
            console.log(`${this.f}领取吃饭翻倍视频奖励获得：${datas.data.amount}金币`)
        } else {
    ...
    async comic() {
        if (nowhour == 8 && nowMin < 30) {
            this.readiid = 1
        } else if (nowhour == 9 && nowMin < 30) {
            this.readiid = 3
        } else if (nowhour == 10 && nowMin < 30) {
            this.readiid = 5
        } else if (nowhour == 11 && nowMin < 30) {
            this.readiid = 10
        } else if (nowhour == 12 && nowMin < 30) {
            this.readiid = 15
        } else if (nowhour == 13 && nowMin < 30) {
            this.readiid = 25
        }
        let headers = {
            "Content-Type": "application/json",
        ...
        if (datas.err_no == 10009) {
            ...
            console.log(`${this.f}看漫画奖励获得：${datas.data.amount}金币`)
        } else {
    ...
    async viewvideo1() {
        ...
        if (datas.err_tips == '今日看广告领金币已达到上限，休息一下~明天再来看吧') {
            ...
            console.log(`${this.f}刷视频成功，获得${datas.data.amount}金币`)
            await this.viewvideo2()
        } else if (datas.err_no == 10009) {
            await this.viewvideo1()
        } else {
            await this.viewvideo2()
        }
    ...
    async viewvideo30() {
        ...
        //  console.log(this.f,datas)
        if (datas.err_no == 0) {
            console.log(`${this.f} 刷视频30秒成功，获得${datas.data.score_amount}金币`)
        } else if (datas.err_no == 10009) {
            console.log(`${this.f} 刷视频30秒 失败，原因 ${datas.err_tips}(被识别)`)
        } else if (datas.err_no != 0) {
            console.log(datas)
            console.log(`${this.f}刷视频30秒 失败，即将重试`)
            view_num++
            if (view_num <= 3) {
    ...
    async shopsignin() {
        ...
        if (datas.err_no == 0) {
            console.log(`${this.f}商城签到成功，获得${datas.data.score_amount}金币`)
        }
    }
    //浏览商品15秒
    async shop15() {
        ...
        if (datas.err_no == 10009) {
            ...
            console.log(`${this.f}浏览商城成功，获得${datas.data.amount}金币`)
        } else if (datas.err_no == 10003) {
            console.log(`${this.f}浏览商城结果，${datas.err_tips}`)
        }
    }
    //搜索得金币
    async search() {
        ...
        if (datas.err_no == 10009 || 10002) {
            if (datas.err_tips == '设备或账号已参加过活动啦，请更换设备或账号再来尝试~') {
                // console.log(datas)
                console.log(`${this.f} 搜索得金币 ${datas.err_tips}`)
            } else {
                await this.search()
            }
        } else if (datas.err_no == 0) {
            console.log(`${this.f}完成搜索任务成功，获得${datas.data.amount}金币`)
        } else if (datas.err_no == 10003) {
            console.log(`${this.f}完成搜索任务结果，${datas.err_tips}`)
        }
    }
    //看本地视频
    async localvideo() {
        ...
        console.log(this.f, datas)
        if (datas.err_no == 0) {
            console.log(`${this.f}观看本地视频120s成功`)
        } else if (datas.err_no != 0) {
...
!(async () => {
    ...
    if ($.cookie_list.length < 999) {
        ...
        if (nowhour == 9 && nowMin < 30) {
            await $.Multithreading('signin')  // 签到
            await $.Multithreading('shopsignin')  // 商城签到
        }
        if (nowhour == 8 && nowMin < 30 || nowhour == 11 && nowMin >= 30 || nowhour == 18 && nowMin < 30 || nowhour == 21 && nowMin < 30) {
            await $.Multithreading('eatmeals') // 吃饭领金币 后面翻倍
        }
        if (nowhour == 10 && nowMin < 30 || nowhour == 13 && nowMin < 30 || nowhour == 17 && nowMin < 30 || nowhour == 19 && nowMin < 30 || nowhour == 20 && nowMin < 30) {
            await $.Multithreading('search')  // 搜索得金币
        }
        if (nowhour == 8 && nowMin < 30 || nowhour == 9 && nowMin < 30 || nowhour == 10 && nowMin < 30 || nowhour == 11 && nowMin < 30 || nowhour == 12 && nowMin < 30 || nowhour == 13 && nowMin < 30) {
            await $.Multithreading('comic')  // 漫画阅读
        }
        if (nowhour == 20 && nowMin < 30) {
