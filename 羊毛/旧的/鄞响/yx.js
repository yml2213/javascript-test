...
const tenantId = 61
const client_id = 10014
let channel = ["63574148fe3fc1680f583f90", "63574156ad61a45c31a182c4", "6374b31bfe3fc1747d4d80ce", "6357413db40eef0e50aafec0", "635747e57fc12c04cf0e4ab2", "635747f6b40eef0e50aafef3", "6357495df0edc004ccfe1f5f", "63758ce2ad61a40b77d50485", "6375cc46fe3fc1747d4d86b5", "63574bdc7fc12c04cf0e4ae2", "6375f048ad61a40b77d5083f"]
// let channel = ["635741273f9a0904cdc8c4fb"]
...
class UserInfo {
    ...
    // 635741273f9a0904cdc8c4fb
    // https://vapp.tmuyun.com/api/article/channel_list?channel_id=635741273f9a0904cdc8c4fb&isDiFangHao=false&is_new=true&list_count=0&size=50
    async getArticleList(paramIn = {}) { // 获取文章列表
        let paramOut = {}
        try {
            let urlParam = {
                fn: 'getArticleList',
    ...
    // https://vapp.tmuyun.com/api/article/channel_list?channel_id=638826519f167d19a7f3d991&isDiFangHao=false&is_new=true&list_count=0&size=20
    async getArticleList_2(paramIn = {}) { // 获取文章列表2
        let paramOut = {}
        try {
            let urlParam = {
                fn: 'getArticleList',
                method: 'get',
                url: `https://vapp.tmuyun.com/api/article/channel_list?channel_id=638826519f167d19a7f3d991&isDiFangHao=false&is_new=true&list_count=0&size=50`,
    ...
    async createComment(article) { // 评论文章
        let paramOut = {}
        try {
    ...
    // https://vapp.tmuyun.com/api/article/detail?id=5006026
    async detail(article) { // 模拟阅读
        let paramOut = {}
        try {
            let urlParam = {
                fn: 'detail',
            ...
            //console.log(result)
            if (result.code == 0) {
                console.log(`模拟阅读[${article.id}]: ${result.message}`)
            } else {
    ...
    async getCommentList(article) { // 删除评论
        let paramOut = {}
        try {
            let urlParam = {
                fn: 'getCommentList',
    ...
    // https://vapp.tmuyun.com/api/user_mumber/numberCenter?is_new=1
    async getTaskList() {
        let paramOut = {}
        try {
            let urlParam = {
            ...
            if (result.code == 0) {
                ...
                // await this.getArticleList_2()  // 获取文章列表
                for (let task of taskList.filter(x => !x.completed)) {
                    for (let i = task.finish_times; i < task.frequency; i++) {
                        let art = null
                        switch (task.member_task_type) {
                            case 2: break//新闻资讯阅读 break
                            case 4: //新闻资讯评论
                                // if (!this.articleList.length) break
                                // let a = randomInt(4, 50)
                                // art = this.articleList[a]
                                // console.log(art)
                                // await this.createComment(art)  // 评论文章
                                // await this.getCommentList(art) // 删除评论 
                                break
                            case 5: //新闻资讯点赞
                                if (!this.articleList.length) break
                                art = this.articleList[i]
                                await this.detail(art)
                                await this.like(art)
                                break
                ...
                console.log(`状态失败: ${result.message}`)
            }
        } catch (e) {
            console.log(e)
        } finally {
