...
class Task {
    ...
    async getUserInfo() {
        ...
        return this.catchGet(name, options).then(async (result) => {
            doubleLog(`账号[${this.userInfo.index}]   ${result.data.rst.nick_name}, 手机号: ${$.phoneNum(result.data.rst.mobile)}, 积分 ${result.data.rst.total_integral}, 等级 ${result.data.rst.grade} ${result.data.rst.grade_name}`);
...
class Api {
    ...
    async httpGet(path, headers) {
        ...
        return await this.httpRequest($.mergeObjs(options, o));
    }
    ...
    async httpRequest(options, type = false) {
...
// 新的 env 函数, 增加自定义功能 yml-11.12改  未合并
