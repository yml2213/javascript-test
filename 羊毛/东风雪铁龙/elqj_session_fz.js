...
const ckNames = ['elqj_fz'];                //支持多变量
const crypto = require('crypto-js');

//====================================================================================================

let pay_name = '杨梦磊';
let pay_email = '83w3s7tscztz@qiott.com';
...
async function my_task(cks) {
    ...
    for (let i = 0; i < result.length; i++) {
        ...
        for (let j = 0; j < result[i].length; j++) {
            let userList = [];
            // console.log(result[i][j])  //  单独一个ck
            let ck_ = result[i][j];
            // console.log(`==========读取了 ${ck_}===========`)
            userList.push(new UserClass(ck_));
        ...
        await $.wait($.randomInt(120, 180));  // 每一组间隔 2 -3 分钟
    }
...
!(async () => {
    ...
    // console.log($.read_env())
    await my_task($.read_env());
