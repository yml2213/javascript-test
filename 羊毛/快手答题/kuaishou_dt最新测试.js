...
class UserClass {
    ...
    async start_dt() { // 开始答题 
        ...
        if (resp.result == 1 && resp.data.roundId) {
            ...
            this.ckFlog = true
        } else if (resp.code == 103703) {
            $.log(`${this.idx}: fn -- ${resp.msg}`)
    ...
    async upload(question, opt, roundId, index, answer, nswer_index) { // 上传答案 
        let options = {
            fn: '上传答案',
            method: 'post',
    ...
    async next(index, question, opt, roundId) { // 答题逻辑
        ...
        if (database_data) {
            ...
            console.log(`================`)
            console.log(answer_index, answer)
            console.log(question, opt, roundId, index, answer, answer_index)
            console.log(`================`)
            await this.upload(question, opt, roundId, index, answer, answer_index)
        } else {
            $.log(`${this.idx}: gpt 时间...`)
    ...
    async to_gpt(question, opt) {
        ...
        // let res2 = await this.gpt3(data)
        let resArr = [res1]
        console.log(resArr)
        let gpt4_data = `你是一个总结机器人,我会给你选项,再加上你的理解   返回问题的正确答案; 参考选项${resArr},  需回答题目: ${question},  题目选项:${opt};请返回我正确答案;使用以下json格式返回:{answer:xxxxx}`

        try {
            let res4 = await this.gpt3(gpt4_data)
            // console.log(res4)
            if (res4) {
                let aa = JSON.parse(res4)
                // console.log(aa)
                this.answer = aa.answer
    ...
    async checkDatabase(question, opt) {
        ...
        // console.log(resp)
        if (resp.code == 200) {
            $.log(`${this.idx}: ${this.question} 数据库有这个题鸭`)
            return resp.data
