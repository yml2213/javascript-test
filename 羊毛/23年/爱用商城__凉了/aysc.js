...
-------------------  青龙-配置文件-复制区域  -------------------
# 爱用商城
export aysc=" userId # ucAccountId # token @ userId # ucAccountId # token "
...
let num_ = '1'          // 刷积分次数
let point='100000'         // 每次刷多少  10000:1
//====================================================================================================
...
class UserClass {
    constructor(ck) {
        ...
        this.ucAccountId = this.ck_[1]
        this.token = this.ck_[2]
        this.ts13 = $.ts(13)
    ...
    async addPoint(num) {
        ...
        let a = `{"userId":"${this.userId}","point":${point},"needReport":0}${this.salt}`
        console.log(a)
    ...
    async check() {
        ...
        // console.log(res)
        if (res.code == 'sc0') {
            $.log(`${this.idx}:${options.fn}, 当前积分: ${res.data.pointTotal}==${res.data.pointTotal / 10000}元`, {notify: true})
        } else console.log(`${options.fn}: 失败,  ${JSON.stringify(res)}`)
    }
