...
-------------------  青龙-配置文件-复制区域  -------------------
# 台陆通 
export tlt_pwd=" phone # pwd @ phone # pwd "  
...
class UserClass {
    ...
    async login() {
        let sign = CryptoJS.MD5(`loginType=1&mobile=${this.phone}&nonce_str=${this.ts13}&platform=1&pwd=${this.pwd}&tenantId=bms2kf0iojdfomqd4r&timestamp=${this.ts13}&key=p3XyivWMO0QCD8RqEE14gZZtamS24v6e`).toString().toUpperCase()
        let options = {
            fn: '登录',
            method: 'post',
            url: `https://api.dalutone.cn/login`,
            headers: this.hd,
            form: {
                "nonce_str": this.ts13,
                "loginType": "1",
                "mobile": this.phone,
                "tenantId": "bms2kf0iojdfomqd4r",
                "sign": sign,
                "pwd": this.pwd,
                "platform": "1",
                "timestamp": this.ts13
        ...
        if (res.code == 200) {
            ...
            $.log(`${this.idx}: ${options.fn}, ${res.msg}!`)
            await $.wait($.randomInt(3, 5))

            await this.loginMall()

            this.ckFlog = true
        } else if (res.code == 500) {
            $.log(`${this.idx}: ${options.fn} 失败:${res.msg}`, { notify: true }), this.ckFlog = false
    ...
    async loginMall() {
        let options = {
            fn: '登录商城',
            method: 'post',
            url: `https://api.dalutone.cn/loginMall`,
            headers: {
                "Host": "api.dalutone.cn",
                "platform": "android",
                "appversion": "1.30",
            ...
            json: 'https://api.dalutone.cn/loginMall'
        }
        // console.log(options)
        let res = await $.request(options)
        // console.log(res)
        if (res.code == 200) {
            this.accessToken_shop = res.data.accessToken
            $.log(`${this.idx}: ${options.fn}, ${res.msg}!`)
            await $.wait($.randomInt(3, 5))
            await this.doSign()
        } else console.log(`${options.fn}: 失败,  ${JSON.stringify(res)}`)
