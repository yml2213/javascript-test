...
//====================================================================================================
let code_tips = `1. 当前版本:sig4_v1版本, 所有旧版不推荐跑了 \n2. 凡是倒卖卡密者,一律封卡密`
...
class UserClass {
    ...
    async upload(question, opt, roundId, index, answer, nswer_index, type = 'database') { // 上传答案 
        ...
        let sig4 = await this.getSig4(this.ck, roundId, index, nswer_index)
        // console.log(sig4)
        ...
        let options = {
            fn: '上传答案',
            method: 'post',
            url: `https://encourage.kuaishou.com/rest/n/encourage/game/quiz/round/answer/upload?__NS_sig4=${sig4}&sigCatVer=1`,
            headers: {
                'Host': 'encourage.kuaishou.com',
                'User-Agent': this.ua,
                'Accept': '*/*',
    ...
    // roundId
    async getSig4(ck, roundId, index, answer) {
        // let sig3str = await this.getSig3str(ck, roundId, index, answer)
        // console.log(sig3str)

        const options = {
            method: 'post',
            url: `http://81.70.196.85:3000/getSig4`,
            headers: {
                'Content-Type': 'application/json',
                // "voucherCode": kuaishou_dt_code
            },
            json: {
                'Cookie': ck,
                'body': `{"roundId":"${roundId}","index":${index},"answer":${answer}}`
            }
        }

        let resp = await $.request(options)
        // console.log(resp)
        // console.log(resp.split('__NS_sig4=')[1].split('&sigCatVer')[0])
        return resp.split('__NS_sig4=')[1].split('&sigCatVer')[0]
        // if (resp.code == 200) {
