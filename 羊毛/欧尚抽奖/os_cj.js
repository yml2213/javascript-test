...
-------------------  青龙-配置文件-复制区域  -------------------
# 欧尚抽奖 
export os_cj="token # openid @ token # openid"  
...
class UserClass {
    ...
    async index() {
        ...
        // token:  user:app:token:15614832213228f82d3fa0c46d690b418da7a8389e0
        // let ts = 20230813192540
        let ts = this.generateTimestamp()
        let a = `${ts}js_apibf231becbb1cdda42572a8170b0c374agameindexappAgent=2&game_id=98&token=${this.token}&tokenAgent=1`
        let sign = CryptoJS.MD5(a).toString().toUpperCase()
        // console.log(sign)
        let options = {
            fn: '抽奖次数',
            method: 'post',
            url: `https://bac.oushangstyle.changan.com.cn/apph5/h5/2.0/common/game/index`,
            headers: this.hd,
            form: {
                "game_id": "98",
                "token": this.token,
        ...
        if (res.code == 200) {
            if (res.data.num) {
                $.log(`${this.idx}: ${options.fn}, 剩余: ${res.data.num} 次, 开始抽奖`)
                for (let index = 0; index < res.data.num; index++) {
                    await this.lottery(index)
                    await $.wait($.randomInt(7, 10))

                }
            } else {
                $.log(`${this.idx}: ${options.fn}, 剩余: ${res.data.num} 次, 去跑拉新吧`)
            }
            this.ckFlog = true
        } else console.log(`${options.fn}: 失败,  ${JSON.stringify(res)}`), this.ckFlog = false
    }
    ...
    async lottery(index) {
        // let ts = 20230813195121
        let ts = this.generateTimestamp()
        let type = 'lottery'
        let a = `${ts}js_apibf231becbb1cdda42572a8170b0c374agame${type}appAgent=2&game_id=98&openid=${this.openid}&token=${this.token}&tokenAgent=1`
        let sign = CryptoJS.MD5(a).toString().toUpperCase()
        // console.log(a)
        // console.log(sign)
        let options = {
        ...
        // console.log(res)
        if (res.code == 200) {
            $.log(`${this.idx}: 第 ${index + 1} 次抽奖, ${res.data.msg}`)
...
function Env(name) {
    return new class {
        ...
        json2str_md5(obj, c = '&', encode = false) {
            let ret = []
            for (let keys of Object.keys(obj).sort()) {
                let v = obj[keys]
                if (v && encode) v = encodeURIComponent(v)
            ...
            let a = CryptoJS.MD5(ret.join(c)).toString()
            return a
        }
