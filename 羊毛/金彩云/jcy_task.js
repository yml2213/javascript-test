...
class User {
    ...
    async welfare() {
        try {
            ...
            const options = {
                method: "get",
                url: `https://op-api.cloud.jinhua.com.cn/api/welfare/info`,
                headers: this.h5_hd,
            }
            // console.log(options)
            let {res} = await requestPromise(options)
            // console.log(res)
            if (res.code === 0) {
                this.log(`当前金币: ${res.data.gold}, 当前余额: ${res.data.cash / 100} 元`, 1)
            } else {
                this.log(res)
            }
        } catch (error) {
    ...
    async onlinetask() {
        try {
            ...
            const options = {
                method: "get",
                url: `https://op-api.cloud.jinhua.com.cn/api/welfare/online`,
                headers: this.h5_hd,
            }
            // console.log(options)
            let {res} = await requestPromise(options)
            //  this.log(res)
            if (res.code === 0) {
                for (let re of res.data.list) {
                    //  console.log(aa)
                    if (re.complete === 0) {
                        await this.get_online_gold(re.id, re.score)
                    } else {
                        // this.log(`在线任务 ${re.id} -- 完成!`)
                    }
                }
            } else {
                this.log(res)
    ...
    // https://op-api.cloud.jinhua.com.cn/api/welfare/task/challenge/getGold
    async get_online_gold(id, score) {
        try {
            let ts13 = ts(13)
            let access_nonce_str = `${randomString(8)}-${randomString(4)}-${randomString(4)}-${randomString(4)}-${randomString(12)}`
            ...
            const options = {
                method: "post",
                url: `https://op-api.cloud.jinhua.com.cn/api/welfare/online/getGold`,
                headers: this.h5_hd,
                json: {"id": id}
            }
            //console.log(options)
            ...
            // console.log(res)
            if (res.code === 0) {
                this.log(`领取在线时长奖励成功: 领取 ${score} 金币`)
                await wait(3, 5)
            } else {
                this.log(res)
            }
    ...
    async challenge_task() {
        try {
            ...
            const options = {
                method: "get",
                url: `https://op-api.cloud.jinhua.com.cn/api/welfare/task/challenge`,
                headers: this.h5_hd,
            }
            // console.log(options)
            let {res} = await requestPromise(options)
            ...
            if (res.code === 0) {
                for (let re of res.data.list) {
                    if (re.complete === 0 && re.isEnable === 1) {
                        await this.get_challenge_gold(re.id, re.score)
                    } else {
                        // this.log(`阅读新闻数量 任务 ${re.id} -- 完成!`)
                    }
                }
            } else {
                this.log(res)
    ...
    async get_challenge_gold(id, score) {
        try {
            let ts13 = ts(13)
            let access_nonce_str = `${randomString(8)}-${randomString(4)}-${randomString(4)}-${randomString(4)}-${randomString(12)}`
            let str1 = `app_id=${this.wx_app_id}&&auth_id=${this.account_id}&&device_id=${this.device_id}&&id=${id}&&nonce_str=${access_nonce_str}&&source_type=app&&timestamp=${ts13}&&token=${this.token}&&${this.signkey}`.trim()
            ...
            const options = {
                method: "post",
                url: `https://op-api.cloud.jinhua.com.cn/api/welfare/task/challenge/getGold`,
                headers: this.h5_hd,
                json: {"id": id}
            }
            //console.log(options)
            ...
            // console.log(res)
            if (res.code === 0) {
                this.log(`领取 阅读新闻数量 奖励成功: 领取 ${score} 金币`)
                await wait(3, 5)
            } else {
                this.log(res)
            }
    ...
    //阅读新闻任务
    // app_id=wxc097803934a957eb&&auth_id=1f1a7f6db8e00e268fb98b8bc0501751&&device_id=70a8848f-80f3-4e54-8ed3-8b83ee7ea44a&&nonce_str=0e3b1b9c-ae49-464a-81d8-a22c3175d073&&page=1&&pageSize=10000&&source_type=app&&timestamp=1703164096222&&token=02a8ed95f7b4451589a4bd2e5138384a&&3(GMx,4]ISc[nK<#
    async readtasklist() {
        try {
            let ts13 = ts(13)
            // let ts13 = 1703166648605
            // let access_nonce_str = `2916cdee-b850-41f3-a6e3-a323a37e003d`
            let access_nonce_str = `${randomString(8)}-${randomString(4)}-${randomString(4)}-${randomString(4)}-${randomString(12)}`
            let str1 = `app_id=${this.wx_app_id}&&auth_id=${this.account_id}&&device_id=${this.device_id}&&nonce_str=${access_nonce_str}&&page=1&&pageSize=10000&&source_type=app&&timestamp=${ts13}&&token=${this.token}&&${this.signkey}`.trim()
            // console.log(str1)
            let sign = CryptoJS.SHA256(str1).toString()
            // console.log(sign)
            this.h5_hd['access-api-signature'] = sign
            ...
            const options = {
                method: "get",
                url: `https://op-api.cloud.jinhua.com.cn/api/welfare/task/challenge/list?page=1&pageSize=10000`,
                headers: this.h5_hd,
            }
            // console.log(options)
            let {res} = await requestPromise(options)
            // this.log(res)
            if (res.code === 0) {
                this.log(`共找到 ${res.data.list.length} 篇文章!`)
                for (let aa of res.data.list) {
                    if (aa.readComplete === 0) {
                        await this.read_glod(aa.contentId, 'click')
                    }
                    if (aa.praiseComplete === 0) {
                        await this.praise_glod(aa.contentId)
                    }
                    if (aa.commentComplete === 0) {
                        await this.comment_glod(aa.contentId, 'comment')
                    }
                    if (aa.shareComplete === 0) {
                        await this.share_glod(aa.contentId, 'share')
                    }
    ...
    // 阅读金币 https://mapi.jcy.jinhua.com.cn/api/publish/add_content_state_num?id=1506987&operation=click&target&timestamp=1703169433990&noncestr=005c0900384c488bb223024a18687b3d&sign=08489d404ec78f2aebdc484c1cd30160
    async read_glod(id, type = 'click') {
        try {
            let ts13 = ts(13)
            let noncestr = randomString(32)
            let str1 = `id=${id}&noncestr=${noncestr}&operation=${type}&timestamp=${ts13}`.trim()
            let sign = await this.get_sign(str1)

            const options = {
                method: "get",
                url: `https://mapi.jcy.jinhua.com.cn/api/publish/add_content_state_num?id=${id}&operation=${type}&target&timestamp=${ts13}&noncestr=${noncestr}&sign=${sign}`,
                headers: this.hd,
            }
            // console.log(options)
            let {res} = await requestPromise(options)
            // console.log(res)
            if (res.code === 0) {
                this.log(`阅读文章: ${id}, 获得 ${res.data.credits} 金豆`)
                await wait(3, 5)
            } else {
                this.log(res)
            }
    ...
    // 点赞金币  id=1507159&noncestr=c8340b727fc040d7a537edd4ea7d4a2f&timestamp=1703168850647
    // https://mapi.jcy.jinhua.com.cn/api/publish/m_praises_add?id=1507159&timestamp=1703168850647&noncestr=c8340b727fc040d7a537edd4ea7d4a2f&sign=0302b1ed194421ba5b099cf3a5503a20
    async praise_glod(id) {
        try {
            let ts13 = ts(13)
            let noncestr = randomString(32)
            let str1 = `id=${id}&noncestr=${noncestr}&timestamp=${ts13}`.trim()
            let sign = await this.get_sign(str1)

            const options = {
                method: "get",
                url: `https://mapi.jcy.jinhua.com.cn/api/publish/m_praises_add?id=${id}&timestamp=${ts13}&noncestr=${noncestr}&sign=${sign}`,
                headers: this.hd,
            }
            // console.log(options)
            let {res} = await requestPromise(options)
            // console.log(res)
            if (res.code === 0) {
                this.log(`点赞文章: ${id}, 获得 ${res.data.credits} 金豆`);
                await wait(3, 5)
    ...
    // 评论
    async comment_glod(id, type = 'comment') {
        try {
            let ts13 = ts(13)
            let noncestr = randomString(32)
            let str1 = `content_id=${id}&noncestr=${noncestr}&operation=${type}&timestamp=${ts13}`.trim()
            let sign = await this.get_sign(str1)

            const options = {
                method: "get",
                url: `https://mapi.jcy.jinhua.com.cn/api/credit/m_credit_add?content_id=${id}&operation=${type}&timestamp=${ts13}&noncestr=${noncestr}&sign=${sign}`,
                headers: this.hd,
            }
            // console.log(options)
            let {res} = await requestPromise(options)
            // console.log(res)
            if (res.code === 0) {
                this.log(`评论文章: ${id}, 获得 ${res.data.credits} 金豆`)
                await wait(3, 5)
            } else {
                this.log(res)
            }
    ...
    // 分享
    async share_glod(id, type = 'share') {
        try {
            let ts13 = ts(13)
            let noncestr = randomString(32)
            let str1 = `id=${id}&noncestr=${noncestr}&operation=${type}&timestamp=${ts13}`.trim()
            ...
            const options = {
                method: "get",
                url: `https://mapi.jcy.jinhua.com.cn/api/publish/add_content_state_num?id=${id}&operation=${type}&timestamp=${ts13}&noncestr=${noncestr}&sign=${sign}`,
                headers: this.hd,
            }
            // console.log(options)
            let {res} = await requestPromise(options)
            // console.log(res)
            if (res.code === 0) {
                this.log(`分享文章: ${id}, 获得 ${res.data.credits} 金豆`)
                await wait(3, 5)
            } else {
                this.log(res)
            }
