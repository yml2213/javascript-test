...
let pay_name = 'xxxx'
let pay_email = '83w3xxxxxscztz@qiott.com'
//====================================================================================================
...
class UserClass {
    ...
    // http://服务器地址/sms/?api=login&user=用户名&pass=密码  http://api.my531.com/Login/?username=用户名&password=密码
    async login() { // 登录
        let options = {
            fn: '登录',
            method: 'get',
    ...
    // http://服务器地址/sms/?api=getSummary&token=令牌  GET/POST 都行  http://api.my531.com/Balance/?token=登录返回的token
    async userInfo(type) { // 个人信息  type: 1 打印余额 2 直接判断
        let options = {
            fn: '个人信息',
            method: 'get',
            url: `${hostName}/Balance/?token={this.token}&type=json`,
            headers: {},
        ...
        if (resp.code == 1) {
            if (type == 1) {
                ...
                $.log(`${this.idx}: 余额:${resp.data.money} 元`)
                if (this.money < 0.13) {
    ...
    // http://服务器地址/sms/?api=getPhone&token=用户令牌&sid=项目ID&Province=省份  GET/POST 都行  http://api.my531.com/GetPhone/?token=登录返回的token&id=项目ID
    ...
    // http://服务器地址/sms/?api=getMessage&token=用户令牌&sid=项目ID&phone=手机号  http://api.my531.com/GetMsg/?token=登录返回的token&id=项目ID&phone=手机号码
    async getMessage(sid, phone) { // 获取验证码  成功返回验证吗, 失败返回 false 
        ...
        if (resp.code == 1) {
            ...
            const pattern = /^\d{6}$/
            return this.smsCode = pattern.test(resp.data)
        } else if (resp.code == -1 && resp.msg == '等待') {
            $.log(`${options.fn}: 当前手机号:${phone}, 验证吗为还没来, 在等一下下.......`)
            return 1
        }
    }

    // http://服务器地址/sms/?api=cancelRecv&token=用户令牌&sid=项目ID&country_code=国家代码&phone=手机号  GET/POST 都行  http://api.my531.com/Cancel/?token=登录返回的token&id=项目ID&phone=手机号码
    ...
    // http://服务器地址/sms/?api=addBlacklist&token=用户令牌&sid=项目ID&country_code=国家代码&phone=手机号  http://api.my531.com/Addblack/?token=登录返回的token&id=项目ID&phone=手机号码
