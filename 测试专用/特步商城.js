 ...
 * 特步商城  小程序
 *
 * cron 0 0,7 * * *  yml2213_javascript_master/tebu.js
 *
 * 7-8		签到
 *
 * ========= 青龙--配置文件 =========
 * 变量格式: export tebu_data=' ezr-vuid & ezr-st & ezr-s & ezr-userid '   ,多账号用 换行 或 @ 分割
 * 
 */
...
//---------------------------------------------------------------------------------------------------------
let ckStr = ($.isNode() ? process.env.tebu_data : $.getdata('tebu_data')) || '';
let msg, ck;
let ck_status = true;
...
!(async () => {
	let ckArr = await checkEnv(ckStr, "tebu_data");
	await tips(ckArr);
...
async function signin() {
	try {
		...
		if (result?.Result.IsGive == true) {
			DoubleLog(`签到:${result.msg} 🎉,获得:${result.Result.BonusValue}积分 `);
			await wait(3);
		} else if (result.Result.IsGive == false) {
			DoubleLog(`签到:${result.Result.ErrMsg}`);
		} else {
