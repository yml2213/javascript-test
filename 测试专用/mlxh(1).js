...
class UserInfo {
    ...
    async dotask(name) {
        ...
        if (result.code == 200) {
            ...
            for (let o of dataarr) {
                ...
                } else { `账号[${this.index}]  ${result.data[o].ruleName}：已完成`; }
            }
        } else DoubleLog(`${name}: 失败 ❌ 了呢,原因未知!`), console.log(result);
    }
    ...
    // eb9b518e11c3abf605000278553f7a69
    async task(name, action, dataId) {
        this.ts = '1668610053619';
        this.dataId = '339086';
        this.action = '8001';
        // console.log(this.ts);
        // console.log(`556b7f514beb5d1ab1b4e11956ceae8a${this.ts}800133535641`);
        this.md5_2 = MD5_Encrypt(`556b7f514beb5d1ab1b4e11956ceae8a${this.ts}8001${this.dataId}41`);
        ...
        let options = {
            ...
            body: `{"action":${this.action},"dataId":"${this.dataId}","sourceId":41,"ts":"${this.ts}","md5":"${this.md5_2}"}`
        };
        ...
        console.log(result);
        if (result.code == 200) {
            DoubleLog(`账号 ${this.index}  ${name}: 成功，获得积分 ${result.data.point}`);
            await wait(5);
        } else if (result.code == 2006) {
            DoubleLog(`账号 ${this.index}  ${name}:  ${result.msg}`);
            await wait(2);
