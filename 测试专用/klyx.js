 ...
 *
 * ========= 青龙--配置文件 =========
 * 变量格式: export klyx_data='xcx_openid & activeId的值 & 城市名'   ,多账号用 换行 或 @ 分割
 * 抓 关键词  kohler-mini.brandsh.cn  找到 xcx_openid 就行了
 * 
...
//---------------------------------------------------------------------------------------------------------
let ckStr = ($.isNode() ? process.env.klyx_data : $.getdata('klyx_data')) || '';
let msg, ck;
let ck_status = true;
let host = 'kohler-mini.brandsh.cn';
...
!(async () => {
	let ckArr = await Variable_Check(ckStr, "klyx_data");
	await tips(ckArr);
...
async function task_list() {
	...
	// if (result.errcode == 0) {
	// 	DoubleLog(`开始检查任务列表`);
	// 	if (result.data.user_task_list[0].status = '0') {
	// 		DoubleLog(`开始 摇晃手机 舞力全开!`);
	// 		await dance();
	// 		await $.wait(15 * 1000);
	// 		await dance_award();
	// 	} else {
	// 		DoubleLog(`今天已经 摇晃手机 过了`);
	// 	}
	// 	if (result.data.user_task_list[1].status = '0') {
	// 		DoubleLog(`开始 预约视频号直播`);
	// 		await video();
	// 		await $.wait(4 * 1000);
	// 		await video_award();
	// 	} else {
	// 		DoubleLog(`今天已经 预约视频号直播 过了`);
	// 	}
	// 	if (result.data.user_task_list[2].status = '0') {
	// 		DoubleLog(`开始 浏览好物10秒`);
	// 		await browse();
	// 		await $.wait(11 * 1000);
	// 		await browse_award();
	// 	} else {
	// 		DoubleLog(`今天已经 浏览好物10秒 过了`);
	// 	}
	// 	if (result.data.user_task_list[4].status = '0') {
	// 		DoubleLog(`开始 分享活动页面1次`);
	// 		await share();
	// 		await $.wait(4 * 1000);
	// 		await share_award();
	// 	} else {
	// 		DoubleLog(`今天已经 分享活动页面1次 过了`);
	// 	}
	// } else {
	// 	DoubleLog(`任务列表: 失败 ❌ 了呢,原因未知!`);
	// 	console.log(result);
...
async function account() {
	...
	let result = await httpPost(Option, `查询余额`);

	if (result.errcode == 0) {
		DoubleLog(`余额查询:${result.msg}：当前${result.data.account}元！`);
