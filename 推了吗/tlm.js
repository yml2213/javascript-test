 ...
 * 推了吗
 * 
 * cron 45 7-20 * * *  yml2213_javascript_master/tlm.js
 * 
...
async function login() {
	...
	if (result.code == 1) {
		...
		console.log(`\n 登录: 欢迎光临 ${result.data.userinfo.nickname} 🎉  \n ${result.data.userinfo.username} \n`);
		msg += `\n 登录: 欢迎光临 ${result.data.userinfo.nickname} 🎉  \n ${result.data.userinfo.username} \n`;
...
async function article_coin() {
	...
	if (result.code == 1) {
		...
		console.log(`\n 阅读文章--领金币: 欢迎光临 ${result.data.userinfo.nickname} 🎉  \n ${result.data.userinfo.username} \n`);
		msg += `\n 阅读文章--领金币: 欢迎光临 ${result.data.userinfo.nickname} 🎉  \n ${result.data.userinfo.username} \n`;

		token = result.data.userinfo.token;

	} else {
		console.log(`\n 阅读文章--领金币: 失败 ❌ 了呢,原因未知！  ${result} \n`);
		msg += `\n 阅读文章--领金币: 失败 ❌ 了呢,原因未知！  ${JSON.parse(result)} \n `;
		throw new Error(`'喂  喂 ---  阅读文章--领金币 失败 ❌ 了呢 ,别睡了, 起来更新了喂!`);
	}
}
