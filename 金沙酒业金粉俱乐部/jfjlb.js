/**
 * 金粉俱乐部 小程序
 * cron 10 8 * * *  yml2213_javascript_master/jfjlb.js
 * 
 * 积分换实物  ; 详细自己看规则 
 * 
 * 金粉俱乐部 
...
let msg = '';
let ck = '';
let jfjlb_data = process.env.jfjlb_data;
/////////////////////////////////////////////////////////
...
!(async () => {
	...
	if (!(await Envs()))  //多账号分割 判断变量是否为空  初步处理多账号
		...
		console.log(`\n=================== 共找到 ${jfjlb_dataArr.length} 个账号 ===================`)
		if (debug) {
			console.log(`【debug】 这是你的账号数组:\n ${jfjlb_dataArr}`);
		}


		for (let index = 0; index < jfjlb_dataArr.length; index++) {


			let num = index + 1
			console.log(`\n========= 开始【第 ${num} 个账号】=========\n`)

			data = jfjlb_dataArr[index].split('&');
			if (debug) {
				console.log(`\n 【debug】 这是你第 ${num} 账号信息:\n ${data}\n`);
			}
...
function login(timeout = 3 * 1000) {
	...
	return new Promise((resolve) => {
		let url = {
			...
			body: `password=${data[1]}&username=${data[0]}`,
...
async function Envs() {
	if (jfjlb_data) {
		if (jfjlb_data.indexOf("@") != -1) {
			jfjlb_data.split("@").forEach((item) => {
				jfjlb_dataArr.push(item);
			});
		} else {
			jfjlb_dataArr.push(jfjlb_data);
		}
	} else {
		console.log(`\n 【${$.name}】：未填写变量 jfjlb_data`)
		return;
	}
