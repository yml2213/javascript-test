...
class UserClass {
    ...
    async userInfo() {
        ...
        if (resp.errorCode == 200) {
            ...
            this.ckFlog = false
            await $.ql('DisableCk_ByCk', this.token)
...
function Env(name) {
    return new class {
        ...
        read_env(Class) {
            let env_str = process.env[ckName]
            ...
            if (!this.userCount) {
                this.log(`未找到变量，请检查变量 ${ckName}`, { notify: true })
                return false
            }
        ...
        async ql(type, ck_) {
            console.log(`==== ql ====`)
            console.log(ckName)
            const { getEnvs, DisableCk_ByCk, EnableCk_ByCk, getstatus } = require('./ql_yml')
            if (type == 'getEnvs') {
                const envs = await getEnvs(ckName)
                // console.log(envs)
                return envs
            } else if (type == 'DisableCk_ByCk') {
                const DisableCk_ByCkBody = await DisableCk_ByCk(ckName, ck_)
                if (DisableCk_ByCkBody.code == 200) {
                    this.log(`账号${ck_}: 禁用成功  ❌`)
                    return true
                } else return false
            } else if (type == 'EnableCk_ByCk_ByCk') {
                console.log(`==== EnableCk_ByCk_ByCk ====`)
                const DisableCkBody = await EnableCk_ByCk(ckName, ck_)
                if (DisableCkBody.code == 200) {
                    this.log(`账号${eid}: 启用成功 ✅`)
                    return true
                } else return false
            } else if (type == 'getstatus') {
                const strnowstatus = await getstatus(ckName, eid)
                if (strnowstatus == 99) {
                    this.log(`账号${eid}: 未知错误`)
                    return 99
                } else if (strnowstatus == 0) {
                    this.log(`账号${eid}: 状态 -- 已启用 ✅`)
                    return 0
                } else if (strnowstatus == 1) {
                    this.log(`账号${eid}: 状态 -- 已禁用 ❌`)
                    return 1
                }
            }
