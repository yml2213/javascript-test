/*
吉利汽车  app               cron 10 7,10,17 * * *  jlqc.js
...
class UserInfo {
    ...
    async signIn() {
        ...
        // console.log(res);
        if (res.code == 'success') {
            this.tgLog(`账号 ${this.index}  ${name}:  成功, 获得积分${res.data.score}`);
        } else if (res.code == 'error.geely.user.already.signed') {
            this.tgLog(`账号 ${this.index}  ${name}:  ${res.message}`);
        } else this.tgLog(`账号[${this.index}]  ${name} 失败 ❌ 了呢`), console.log(res);
    }
    ...
    async task() {
        ...
        // console.log(res);
        if (res.code == 'success') {
            this.tgLog(`账号 ${this.index}  ${name}:  成功`);
            this.id = res.data;
            await $.wait(10);
            await this.comment();
        } else this.tgLog(`账号[${this.index}]  ${name} 失败 ❌ 了呢`), console.log(res);
    ...
    async point() {
        ...
        // console.log(res);
        if (res.code == 'success') {
            this.tgLog(`账号 ${this.index}  ${name}: : 积分 ${res.data.availablePoint}==${res.data.availablePoint / 10}元`);
        } else this.tgLog(`账号[${this.index}]  ${name} 失败 ❌ 了呢`), console.log(res);
    }
    ...
    async get_text() {
        ...
        } else this.tgLog(`账号[${this.index}]   失败 ❌ 了呢`), console.log(res);
    }
