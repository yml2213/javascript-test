 ...
 * 感谢所有测试人员
 * ========= 青龙--配置文件 =========
 * 变量格式: export mtyd_data='x-auth-token @ x-auth-token'   ,多账号用 换行 或 @ 分割
...
const $ = new Env("mate云端");
const notify = $.isNode() ? require("./sendNotify") : "";
const Notify = 1 		//0为关闭通知,1为打开通知,默认为1
const debug = 0			//0为关闭调试,1为打开调试,默认为0
//---------------------------------------------------------------------------------------------------------
let ckStr = ($.isNode() ? process.env.mtyd_data : $.getdata('mtyd_data')) || '';
let msg, ck;
let ck_status = true;
...
!(async () => {
	if (typeof $request !== "undefined") {  // 严格不相等
		...
		let ckArr = await checkEnv(ckStr, "mtyd_data");
		await tips(ckArr);
	...
	if ($request.url.indexOf("activity/normal/sign_in/get_sign_in_user") > -1) {
		...
		if (ckStr) {
			if (ckStr.indexOf(ck) == -1) {  // 找不到返回 -1
				ckStr = ckStr + "@" + ck;
				$.setdata(ckStr, "mtyd_data");
				ckList = ckStr.split("@");
				$.msg($.name + ` 获取第${ckList.length}个 ck 成功: ${ck}`);
			}
		} else {
			$.setdata(ck, "mtyd_data");
...
async function do_task(id) {
	try {
		...
		let result = await httpPost(url, `做任务`);

		if (result?.code == 0) {
			DoubleLog(`做任务: ${result?.message}`);
			await wait(3);
		} else if (result.code == 300011) {
			DoubleLog(`做任务: ${result?.message}`);
		} else {
			DoubleLog(`做任务: 失败 ❌ 了呢,原因未知!`);
			console.log(result);
		}
	} catch (error) {
		console.log(error);
