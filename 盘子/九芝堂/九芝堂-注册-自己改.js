...
//-------------------- 配置区域 --------------------
const reg_num = '3'  //注册数量
const regcode = '738642' //邀请码
const pw = 'jzt123654'  //密码
// -----------------------------------------------
...
// 注册
async function jztreg(name) {
	console.log(`\n开始 第${index + 1}次${name}`);
	await get_code('获取验证码');
	try {
		let url = `https://api.jiuzhitangfre.com//register`
		phone_code = await phone('获取手机号')
		console.log(phone_code);
		let body = `username=${phone_code}&captcha=${code}&password=${pw}&password2=${pw}&code=${regcode}`
...
async function get_code(name) {
	console.log(`\n开始 ${name}`);
	try {
		let url = `https://api.jiuzhitangfre.com//verify`
