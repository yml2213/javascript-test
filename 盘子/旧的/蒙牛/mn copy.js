/*
注册地址: https://app.mengniumuchangdny.com/
邀请码: 471836
需要实名后才能运行
手机号&密码
*/
...
let userCookie = ($.isNode() ? process.env.mnck : $.getdata('mnck')) || '';
let userList = []
let userIdx = 0
let userCount = 0
...
class UserInfo {
	...
	async verify() {
		try {
			let url = `https://app.mengniumuchangdny.com/api/v1/verify?`;
			let body = ``;
			let h = {
			...
			if (result.code == 0) {
				...
				// console.log(`账号${this.ck[0]}  ${result.message} `)
				await $.wait(3300)
				await this.cashck()
			}
		} catch (e) {
	...
	async cashck() {
		try {
			let url = `https://app.mengniumuchangdny.com/api/v1/login`;
			let body = `{"password":"${this.ck[1]}","username":"${this.ck[0]}","vcode":"${this.captcha}"}`;
			//console.log(body)
			let urlObject = popu(url, h, body)
	...
	async sign() {
		try {
			let url = `https://app.mengniumuchangdny.com/api/v1/sign`;
			let body = `{}`;
			let h = {
			...
			let result = httpResult;
			if (result.code == 0) {
				console.log(`账号${this.ck[0]} ${result.msg} ${result.data}`)
	...
	async zp() {
		try {
			let url = `https://app.mengniumuchangdny.com/api/v1/prize`;
			let body = `{}`;
			let h = {
				"authorization": `Bearer ${this.x}`
			}
	...
	async total() {
		try {
			let url = `https://app.mengniumuchangdny.com/api/v1/user-center?`;
			let body = ``;
			let h = {
				"authorization": `Bearer ${this.x}`
			}
			...
			//  console.log(result)
			if (result.code == 0) {
				console.log(`账号${this.ck[0]} 可提现 ${result.data.usable_balance} 元 `)
			}
		} catch (e) {
			console.log(e)
		} finally {
