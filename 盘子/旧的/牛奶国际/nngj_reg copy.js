...
// }
// resp = request.get('https://raw.githubusercontent.com/yml2213/javascript/master/jieda/jieda.js', headers = headers)
// // captcha_img = resp.body
// console.log(resp);


const $ = new Env("牛奶国际-注册机");
...
async function start() {
	...
	http.get('http://niunai.mkjng.top:1003/captcha/flat?MpKTuXFp', function (req, res) {
		var imgData = '';
		...
		req.on('end', function () {
			...
			fs.writeFile(path2, imgData, 'binary', function (err) {  //path为本地路径例如public/logo.png
				if (err) { console.log(`保存出错！,${err}`) } else {
					console.log('保存成功!')
					path_code = `${dirPath}/code.png`
					file = fs.open(path_code)
					console.log(file);
					wait(3)
					recognition_coed('识别', path_code)
	...
	// 识别验证码
	async function recognition_coed(name, file) {
		console.log(`\n开始 ${name}`);
		try {
			let Options = {
				url: `http://81.70.196.85:9898/ocr/file`,
				headers: {
