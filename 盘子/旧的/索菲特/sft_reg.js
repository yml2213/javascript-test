...
let regcode = '243578' 			//邀请码  ['b28cc2,17adf1,c02f59,c06005,cd5eb3' ]
let host = 'app.suofeitejiudianzgt.com/'
let hostname = 'https://' + host
// -----------------------------------------------


const $ = new Env("索菲特-注册机");
const alias_name = 'sft'
...
async function start() {
	index = 0
	await sftreg('开始注册', index, regcode);
	for (index; index < reg_num; index++) {
		await sftreg('开始注册', index, new_regcode);
	}
	console.log(`账号信息\n\n`);
...
// 注册  https://api.suofeitejiudinoxe.com//register
async function sftreg(name, index, regcode) {
	...
	try {
		let Options = {
			url: `https://api.suofeitejiudinoxe.com//register`,
			headers: {
...
// 登录   httpPost    https://api.suofeitejiudinoxe.com//login
async function login(name, pw, phone_code) {
	DoubleLog(`\n开始 ${name}`);
	try {
		let Options = {
			url: ` https://api.suofeitejiudinoxe.com//login`,
			headers: {
				"content-type": "application/x-www-form-urlencoded",
			},
			form: {
...
// 获取验证码  https://api.suofeitejiudinoxe.com//verify
async function get_code(name) {
	// console.log(`\n开始 ${name}`);
	try {
		let Options = {
			url: `https://api.suofeitejiudinoxe.com//verify`,
			headers: {
				"content-type": "application/x-www-form-urlencoded",
			},
		};
