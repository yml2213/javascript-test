/*
走进海南  盘子               cron 10 7,10 * * *  zjhn.js

========= 青龙--配置文件--贴心复制区域  ========= 
# 走进海南
export zjhn=' phone # pwd  @ phone # pwd '  
...
class UserInfo {
    ...
    async login() {
        ...
        let path = '/api/PlayerVue/Login';
        let data_ = this.get_data((path).toLowerCase());
        let auth_key = encodeURIComponent(this.get_auth_key(path, data_));

        let options = {
            method: "post",
            url: `https://api.hinan8.com${path}?b8ef038bdc27fc47a5087ce0f63e7622=${data_}&auth_key=${auth_key}`,
            headers: {
                'authority': 'api.hinan8.com',
                'token': '',
                'x-forwarded-for': '112.36.8.90',
                'x-requested-with': 'XMLHttpRequest',
                'content-type': 'application/json'
            },
            body: `{"password":"${this.pwd}","username":"${this.phone}","checked":true,"show":false,"url":"https://hi-www.kinkino.com/DownApp","IsKeepPWD":false}`
        };
    ...
    async user_info() {
        ...
        let path = '/api/Minmetals/postUserInfo';
        let data_ = this.get_data((path).toLowerCase());
        let auth_key = encodeURIComponent(this.get_auth_key(path, data_));
        ...
        // console.log(res);
        if (res.code == 200) {
            doubleLog(`账号[${this.index}] ${name}: 可提现 ${res.data.freedAccount} 元`);
        } else doubleLog(`账号[${this.index}]  ${name} 失败 ❌ 了呢`), console.log(res);
    }
    ...
    get_data(path) {
        ...
        let e = F();
        let a = "0";
        let p = "eb15bc2f2b6a1ec91c2411be7a1501a5";
        // let o = Z()(n + "-" + i + "-" + e + "-" + a + "-" + p);
        let o = MD5Encrypt(path + "-" + i + "-" + e + "-" + a + "-" + p);
        return "".concat(i, "-").concat(e, "-").concat(a, "-").concat(o);
        function F() {
    ...
    get_auth_key(path, data_) {
        // let e = "1668661370";
        let e = $.ts(10);
        let a = path + "?" + "b8ef038bdc27fc47a5087ce0f63e7622=" + data_ + e;
        // console.log(a);
        let u = aa(a);
        // console.log("u: ", u);
        let t = encodeURI(u);
