/*
通晟泰 盘子            cron 22 8,12 * * *  tct.js

https://www.tsthhtysy.com/h5/index.html#/login
...
class UserInfo {
    ...
    async login() {
        ...
        let time = $.ts(10)
        let sign = MD5_Encrypt(`appId=tea&nonceStr=PfVihvHe&time=${time}e3de382b2bab1232s`)

        let options = {
            method: "post",
            url: `https://www.tsthhtysy.com/api/v1/site/login?appId=tea&nonceStr=PfVihvHe&time=${time}&sign=${sign}`,
            headers: {
                // 'Cookie': this.cookie,
            ...
            form: {
                ...
                'group': 'app',
                'drive': '1',
                'phone_identifier': '3252a5059de1e17d5a063907a4e95420'
            }
        }
        // console.log(options)
    ...
    async doSign() {
        ...
        let options = {
            method: "post",
            url: `https://www.tsthhtysy.com/api/v1/member/sign?appId=tea&nonceStr=PfVihvHe&time=${time}&sign=${sign}`,
            headers: {
        ...
        if (res.code == 200) {
            ...
            this.cusLog(`${this.idx}  ${name}:  ${res.message}, 也可能是没有实名认证!`)
        } else this.cusLog(`${this.idx} ${name} 失败 ❌ 了呢`), console.log(res)
    }
    ...
    async getUserInfo() {
        ...
        let options = {
            method: "get",
            url: `https://www.tsthhtysy.com/api/v1/member/member?appId=tea&nonceStr=PfVihvHe&time=${time}&sign=${sign}`,
            headers: {
                'access-token': this.access_token
            }
        }
        ...
        // console.log(res)
        if (res.code == 200) {
            this.cusLog(`${this.idx}  ${name}:  余额 ${res.data.account.user_money}, 电能积分 ${res.data.account.user_integral}`)
            await $.wait(2)
        } else this.cusLog(`${this.idx} ${name} 失败 ❌ 了呢`), console.log(res)
