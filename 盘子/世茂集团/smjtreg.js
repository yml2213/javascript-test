...
@jack 索菲特批量注册  
安装  fs  依赖
账号自动写入到同目录 smjt.txt文件里面
自动创建文件  不需要新建
*/
const $ = new Env("世茂集团批量注册");
let envSplitor = ['@', '\n']
...
let num = 5  //注册数量  最多一次100个  //偶尔会停不下来 会在超出设定数量后 强制停止

let codex = '682513'  //  提供一个初始得邀请码  
///////////////////////////////////////////////////////////////////
...
class UserInfo {
    ...
    async updata(a) {
        ...
        try {
            ...
            fs.appendFile('./smjt.txt', content, { flag: 'a+' }, err => {
                if (err) {
                    console.error(err)
                    return
                }
    ...
    async verify() {
        try {
            let url = `https://api.shimaojituancmg.com//verify`
            let body = ``
            let h = { 'Authorization': 'Bearer ' + this.token }
            let urlObject = popu(url, h, body)
            await httpRequest('get', urlObject)
    ...
    async register() {
        try {
            ...
            let url = `https://api.shimaojituancmg.com//register`
            this.a = getphone()
