...
class UserInfo {
    ...
    async signin(name) { //签到
        ...
        // console.log(res);
        if (res.flag == 1) {
            this.doubleLog(`账号[${this.index}]  ${name}" ${res.msg}, 签到天数 ${res.data.day}, 其他:${res.extra}`);
            await wait(2);
        } else if (res.flag == -1) {
            this.doubleLog(`账号[${this.index}]  ${name}" ${res.msg}`);
            await wait(2);
        } else this.doubleLog(`账号[${this.index}]  ${name} 失败 ❌ 了呢`), console.log(res);
    ...
    async user_info(name) { // 用户信息
        ...
        // console.log(res);
        if (res.flag == 1) {
            this.doubleLog(`账号[${this.index}]   ${res.data.user_nickname}, 手机号: ${utils.phone_num(res.data.user_phone)}, 积分 ${res.data.user_score}, 邀请码 ${res.data.user_invite_code} `);
            await wait(2);
...
function Env(name, e) {
    ...
    return new (class {
        ...
        log(...name) {
            name.length > 0 && (this.logs = [...this.logs, ...name]), console.log(name.join(this.logSeparator));
        }
        done() {
