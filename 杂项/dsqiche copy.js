 ...
 *
 * ========= 青龙--配置文件 =========
 * 变量格式: export dsqc='手机号 & 密码 ' 多账号@ 分割
 * 
 */

const { log } = require("console");

const $ = new Env("DS汽车APP");
const notify = $.isNode() ? require("./sendNotify") : "";
const Notify = 1 		//0为关闭通知,1为打开通知,默认为1
const debug = 0		//0为关闭调试,1为打开调试,默认为0
//---------------------------------------------------------------------------------------------------------
let ckStr = ($.isNode() ? process.env.dsqc : $.getdata('dsqc')) || '';
let msg, ck;
let ck_status = true;
...
!(async () => {
	let ckArr = await checkEnv(ckStr, "dsqc");
	await tips(ckArr);
...
async function signin() {
	let Option = {
		url: `https://act-client.o2o.mpsa.cn/api/v1/activity/sign-in/sign-in?activityId=10020000471&access_token=${accessToken}`,
		headers: {
...
async function task_list() {
	let Option = {
		url: `https://act-client.o2o.mpsa.cn/api/v1/activity/sign-in/assignments?activityId=10020000471&access_token=${accessToken}`,
		headers: {
			'Host': 'act-client.o2o.mpsa.cn',
			'accept-language': 'zh-cn',
		},
	...
	if (result.status == 200) {
		...
		if (result.data[0].status == 1) {
			DoubleLog(`开始 ${result.data[0].bizName}`);
			await article_list();
			await $.wait(3 * 1000);
			await browse();
			await $.wait(randomInt(16000, 21000));
			await browse_award();
		}
		else {
			DoubleLog(`今天已经浏览过了`);
		}
		if (result.data[1].status == 1) {
			DoubleLog(`开始 ${result.data[1].bizName}`);
			await article_list();
			await $.wait(2 * 1000);
...
async function article_list() {
	...
	let result = await httpGet(Option, `文章列表`);

	if (result.status == 200) {
		DoubleLog(`文章列表:${result.message}`);
...
async function browse() {
	...
	let result = await httpGet(Option, `浏览文章`);

	if (result.status == 200) {
		DoubleLog(`浏览文章:${result.message}`);
		await wait(5);
	} else {
		DoubleLog(`浏览文章: 失败 ❌ 了呢,原因未知!`);
		console.log(result);
	}
}
...
async function comment() {
	...
	let result = await httpPost(Option, `评论文章`);

	if (result.status == 200) {
		DoubleLog(`评论文章:${result.message}`);
		await wait(5);
	} else {
		DoubleLog(`评论文章: 失败 ❌ 了呢,原因未知!`);
		console.log(result);
	}
}
...
async function browse_award() {
	...
	let result = await httpPost(Option, `浏览文章奖励`);

	if (result.status == 200) {
		DoubleLog(`浏览文章奖励:${result.message}`);
		await wait(5);
	} else {
		DoubleLog(`浏览文章奖励: 失败 ❌ 了呢,原因未知!`);
		console.log(result);
	}
}
...
async function user_info() {
	...
	if (result.status == 200) {
		DoubleLog(`用户信息:${result.message},手机号${result.data.mobile}`);
		await wait(5);
	} else {
...
async function get_point() {
	let Option = {
		url: `https://mini-app.o2o.mpsa.cn/api/v1/point/account/consumer/point-detail/current-user?access_token=${accessToken}&tenantId=0`,
		headers: {
			'Host': 'mini-app.o2o.mpsa.cn',
			'Accept-Language': 'zh-cn',
		},
	...
	let result = await httpGet(Option, `积分查询`);

	if (result.status == 200) {
		DoubleLog(`积分查询:${result.message},当前积分${result.data.point}`);
		await wait(5);
	} else {
