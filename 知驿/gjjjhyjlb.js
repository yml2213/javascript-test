/**
 * 脚本地址:  https://raw.githubusercontent.com/yml2213/javascript/master/zhiyi/zhiyi.js
 * 转载请留信息,谢谢
 *
 * 知驿  app
 *
 * cron 20 7,12  * * *  yml2213_javascript_master/zhiyi.js
 *
 * 6-12		完成 签到 任务 (圈x v2p兼容测试中)
 *
 * 感谢所有测试人员
 * ========= 青龙--配置文件 =========
 * 变量格式: export zhiyi_data='X-Customer & X-Token @ X-Customer & X-Token '   ,多账号用 换行 或 @ 分割
 * 抓包 club-server/front/member/home  包, 找到 X - Token ,X - Customer 即可
 * ========= 重写 (测试中,有问题群里 @ 我吧) =========
 * url:   club-server/front/member/home
 * 类型:   script-request-header
 * 路径:   https://raw.githubusercontent.com/yml2213/javascript/master/zhiyi/zhiyi.js
 * 域名:   mc.kukahome.com
 * ====================================
...
//---------------------------------------------------------------------------------------------------------
let ckStr = ($.isNode() ? process.env.zhiyi_data : $.getdata('zhiyi_data')) || '';
let msg, ck;
let ck_status = true;
let host = 'kaxin.jieyou.work';
...
async function tips(ckArr) {
	let Version_latest = await Version_Check('zhiyi');
...
!(async () => {
	if (typeof $request !== "undefined") {  // 严格不相等
		...
		let ckArr = await Variable_Check(ckStr, "zhiyi_data");
		await tips(ckArr);
	...
	if ($request.url.indexOf("club-server/front/member/home") > -1) {
		...
		if (ckStr) {
			if (ckStr.indexOf(ck_id) == -1) {  // 找不到返回 -1
				ckStr = ckStr + "@" + ck;
				$.setdata(ckStr, "zhiyi_data");
				ckList = ckStr.split("@");
				$.msg($.name + ` 获取第${ckList.length}个 ck 成功: ${ck}`);
			}
		} else {
			$.setdata(ck, "zhiyi_data");
